...
static mscc_appl_trace_module_t trace_module = {.name = "phy_warmstart"};
...
static cli_cmd_t cli_cmd_table[] = {
    {
     "PHY Warmstart enable", "Enable Warm start for PHY",
     cli_cmd_phy_warmstart_enable, },
    {
     "PHY Warmstart perform",                     "Store all the PHY configurations (port,TS,MACsec) in .json and Load back",
...
static int phy_assign_callout(void)
{
    ...
    mepa_callout_wr.spi_read = meba_spi_read;
    mepa_callout_wr.spi_write = meba_spi_write;
    ;
    return MEPA_RC_OK;
}
    ...
    for (int port_no = 0; port_no < (meba_phy_inst_wr->phy_device_cnt - 1); port_no++) {
        if ((rc = phy_family_detect(meba_phy_inst_wr, port_no, &phy_info)) == MEPA_RC_OK) {
            ...
            case PHY_FAMILY_MALIBU_25G:
                rc = mepa_warmstart_conf_set(meba_phy_inst_wr->phy_devices[port_no],
                                             MEPA_RESTART_WARM);
                break;
            default:
...
static int mepa_drv_create()
{
    ...
    for (int port_no = 0; port_no < meba_phy_inst_wr->phy_device_cnt; port_no++) {
        if (gaphy_family[port_no] == PHY_FAMILY_MALIBU_25G) {
            ...
            meba_phy_inst_wr->phy_devices[port_no] =
                mepa_create(&(mepa_callout_wr), &meba_phy_inst_wr->phy_device_ctx[port_no],
                            &board_conf);
        }
    }
    return MEPA_RC_OK;
            ...
            if (warmstart_enabled) {
                ...
                if (loadjson) {
                    if (process_json_items(meba_phy_inst_wr->phy_devices[port_no], port_no) == 0) {
                        ...
                        if (meba_phy_inst_wr->phy_devices[port_no] &&
                            meba_phy_inst_wr->phy_devices[entry.phy_base_port]) {
                            (void)mepa_link_base_port(meba_phy_inst_wr->phy_devices[port_no],
                                                      meba_phy_inst_wr
                                                          ->phy_devices[entry.phy_base_port],
                                                      entry.map.chip_port);
                        }
                    } else {
