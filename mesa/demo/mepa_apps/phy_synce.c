...
static mscc_appl_trace_module_t trace_module = {.name = "phy_synce_config"};
...
static cli_cmd_t cli_cmd_table[] = {
    {
     "PHY Synce_conf <port_no> <enable|disable>", "syncE configuration for PHY",
...
static cli_parm_t cli_parm_table[] = {
    {
     ...
     CLI_PARM_FLAG_SET, cli_param_synce_ena_dis,
     cli_cmd_synce_configure, },
};

static const char *const src_options_malibu[] = {"line0", "line1", "line2", "line3",  "host0",
                                                 "host1", "host2", "host3", "srefclk"};

static const char *const src_options_viper[] = {"copper", "serdes"};

static const char *const src_options_lan8814[] = {"copper", "serdes", "clock_in_1", "clock_in_2"};

static const char *const dst_malibu[] = {"sckout", "ckout0", "ckout1", "ckout2", "ckout3"};

static const char *const dst_viper[] = {"rcvrclk1", "rcvrclk2"};

static const char *const dst_lan8814[] = {"rcvrclk1", "rcvrclk2"};

static const char *const lan80xx_dst[] = {"rcvrclk1", "rcvrclk2"};

static const float freq_malibu[] = {62.5, 125, 156.25, 161.13, 322.27};

static const float freq_lan80xx[] = {75.0f,  37.50f, 79.58f,  39.79f, 125.0f, 62.50f,
                                     31.25f, 15.62f, 128.90f, 64.45f, 32.22f, 80.56f};

static const float freq_viper[] = {25, 31.25, 125};

static const float freq_lan8814[] = {25, 31.25, 125};

static const char *sq_opt[] = {"none", "link_line0", "link_line1", "link_line2", "link_line3"};

mepa_synce_clock_src_t str_to_src_enum(const char *src)
{
    if (strcmp(src, "line0") == 0)
        return MEPA_SYNCE_CLOCK_SRC_LINE0;
    if (strcmp(src, "line1") == 0)
        return MEPA_SYNCE_CLOCK_SRC_LINE1;
    if (strcmp(src, "line2") == 0)
        return MEPA_SYNCE_CLOCK_SRC_LINE2;
    if (strcmp(src, "line3") == 0)
        return MEPA_SYNCE_CLOCK_SRC_LINE3;
    if (strcmp(src, "host0") == 0)
        return MEPA_SYNCE_CLOCK_SRC_HOST0;
    if (strcmp(src, "host1") == 0)
        return MEPA_SYNCE_CLOCK_SRC_HOST1;
    if (strcmp(src, "host2") == 0)
        return MEPA_SYNCE_CLOCK_SRC_HOST2;
    if (strcmp(src, "host3") == 0)
        return MEPA_SYNCE_CLOCK_SRC_HOST3;
    if (strcmp(src, "srefclk") == 0)
        return MEPA_SYNCE_CLOCK_SRC_SREFCLK;
    if (strcmp(src, "copper") == 0)
        return MEPA_SYNCE_CLOCK_SRC_COPPER_MEDIA;
    if (strcmp(src, "serdes") == 0)
        return MEPA_SYNCE_CLOCK_SRC_SERDES_MEDIA;
    if (strcmp(src, "clock_in_1") == 0)
        return MEPA_SYNCE_CLOCK_SRC_CLOCK_IN_1;
    if (strcmp(src, "clock_in_2") == 0)
        return MEPA_SYNCE_CLOCK_SRC_CLOCK_IN_2;
    if (strcmp(src, "disabled") == 0)
        return MEPA_SYNCE_CLOCK_SRC_DISABLED;
    // Add more as needed
...
{
    if (strcmp(dst, "sckout") == 0)
        return MEPA_SYNCE_CLOCK_DST_SCKOUT;
    if (strcmp(dst, "ckout0") == 0)
        return MEPA_SYNCE_CLOCK_DST_1;
    if (strcmp(dst, "ckout1") == 0)
        return MEPA_SYNCE_CLOCK_DST_2;
    if (strcmp(dst, "ckout2") == 0)
        return MEPA_SYNCE_CLOCK_DST_3;
    if (strcmp(dst, "ckout3") == 0)
        return MEPA_SYNCE_CLOCK_DST_4;
    if (strcmp(dst, "rcvrclk1") == 0)
        return MEPA_SYNCE_CLOCK_DST_1;
    if (strcmp(dst, "rcvrclk2") == 0)
        return MEPA_SYNCE_CLOCK_DST_2;
    // Add more as needed
...
{
    if (freq == 25.0f)
        return MEPA_FREQ_25M;
    if (freq == 31.25f)
        return MEPA_FREQ_31_25M;
    if (freq == 62.5f)
        return MEPA_FREQ_62_5M;
    if (freq == 125.0f)
        return MEPA_FREQ_125M;
    if (freq == 155.52f)
        return MEPA_FREQ_155_52M;
    if (freq == 156.25f)
        return MEPA_FREQ_156_25M;
    if (freq == 161.13f)
        return MEPA_FREQ_161_13M;
    if (freq == 311.04f)
        return MEPA_FREQ_311_04M;
    if (freq == 322.27f)
        return MEPA_FREQ_322_27M;
    if (freq == 75.0f)
        return MEPA_FREQ_75M;
    if (freq == 37.50f)
        return MEPA_FREQ_37_50M;
    if (freq == 79.58f)
        return MEPA_FREQ_79_58M;
    if (freq == 39.79f)
        return MEPA_FREQ_39_79M;
    if (freq == 15.62f)
        return MEPA_FREQ_15_62M;
    if (freq == 128.90f)
        return MEPA_FREQ_128_90M;
    if (freq == 64.45f)
        return MEPA_FREQ_64_45M;
    if (freq == 32.22f)
        return MEPA_FREQ_32_22M;
    if (freq == 80.56f)
        return MEPA_FREQ_80_56M;
    return MEPA_FREQ_125M; // Default/fallback
}

mepa_squelch_src_t str_to_squelch_enum(const char *squelch)
{
    if (strcmp(squelch, "none") == 0)
        return MEPA_SYNCE_NO_SQUELCH;
    if (strcmp(squelch, "link_line0") == 0)
        return MEPA_SYNCE_SQUELCH_LINK_LINE0;
    if (strcmp(squelch, "link_line1") == 0)
        return MEPA_SYNCE_SQUELCH_LINK_LINE1;
    if (strcmp(squelch, "link_line2") == 0)
        return MEPA_SYNCE_SQUELCH_LINK_LINE2;
    if (strcmp(squelch, "link_line3") == 0)
        return MEPA_SYNCE_SQUELCH_LINK_LINE3;
    // Add more as needed
...
static inline void read_input(char *buf) { scanf("%31s", buf); }
...
synce_disable:
    T_D("src : %d, dst : %d, freq : %d, squelch_src : %d, squelch_inv : %d", conf.src, conf.dst,
        conf.freq, conf.squelch.squelch_src, conf.squelch.squelch_inv);
    // --- Apply -----------------------------------------------------------
