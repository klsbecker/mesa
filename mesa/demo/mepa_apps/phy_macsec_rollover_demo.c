...
static mscc_appl_trace_module_t trace_module = {.name = "macsec-rollover-demo"};
...
static const mepa_mac_t peer_macaddress = {
    .addr = {0xff, 0xff, 0xff, 0xff, 0xff, 0xf1}
};
static const mepa_mac_t mku_mac_address = {
    .addr = {0x01, 0x80, 0xC2, 0x00, 0x00, 0x03}
};
...
typedef enum { MACSEC_ROLLOVER_TX = 0, MACSEC_ROLLOVER_RX = 1 } macsec_rollover_dir_t;
...
static void cli_cmd_macsec_rollover_demo(cli_req_t *req)
{
    ...
    if ((rc = mepa_macsec_init_set(meba_macsec_rollover_instance->phy_devices[req->port_no],
                                   &init_data_enable)) != MEPA_RC_OK) {
    ...
    if ((rc =
             mepa_macsec_default_action_set(meba_macsec_rollover_instance->phy_devices[req->port_no],
                                            req->port_no, &default_action_policy)) != MEPA_RC_OK) {
        T_E("[ERROR] Default action configuration failed on port %d\n", port);
        return;
    }

    mepa_macsec_port_t macsec_port = {
        .port_no = req->port_no, .port_id = MACSEC_SECY_ID, .service_id = 0};

    mepa_macsec_sci_t sci = {.mac_addr = peer_macaddress, .port_id = MACSEC_SECY_ID};

    // Create SecY
    mepa_macsec_secy_conf_t secy_conf = {.validate_frames = MEPA_MACSEC_VALIDATE_FRAMES_STRICT,
                                         .replay_protect = TRUE,
                                         .replay_window = 10,
                                         .protect_frames = TRUE,
                                         ...
                                         .current_cipher_suite =
                                             MEPA_MACSEC_CIPHER_SUITE_GCM_AES_256};

    if (cipher_suit == MACSEC_XPN_CIPHER_SUIT) {
        secy_conf.current_cipher_suite = MEPA_MACSEC_CIPHER_SUITE_GCM_AES_XPN_256;
    }

    if ((rc = mepa_macsec_secy_conf_add(meba_macsec_rollover_instance->phy_devices[req->port_no],
    ...
    // Pattern matching
    mepa_macsec_match_pattern_t pattern_match = {.priority = MEPA_MACSEC_MATCH_PRIORITY_HIGH,
                                                 .match = MEPA_MACSEC_MATCH_ETYPE,
                                                 .is_control = TRUE,
                                                 .has_vlan_tag = FALSE,
                                                 .has_vlan_inner_tag = FALSE,
                                                 .etype =
                                                     rollover_direction_egress ? 0x800 : 0x88e5};

    if ((rc = mepa_macsec_pattern_set(meba_macsec_rollover_instance->phy_devices[req->port_no],
                                      macsec_port,
                                      rollover_direction_egress ? MEPA_MACSEC_DIRECTION_EGRESS
                                                                : MEPA_MACSEC_DIRECTION_INGRESS,
                                      MEPA_MACSEC_MATCH_ACTION_CONTROLLED_PORT, &pattern_match)) !=
        MEPA_RC_OK) {
    ...
    if ((rc = mepa_macsec_secy_controlled_set(meba_macsec_rollover_instance
    ...
    if (rollover_direction_egress) {
        ...
        if ((rc = mepa_macsec_tx_sc_set(meba_macsec_rollover_instance->phy_devices[req->port_no],
        ...
        if ((rc = mepa_macsec_rx_sc_add(meba_macsec_rollover_instance->phy_devices[req->port_no],
    ...
    /* Configure Paacket with Ethertype as 0x888E and DMAC address as
     * "0x01:0x80:0xC2:0x00:0x00:0x03" as Control packet - MKPDU packet*/
    ...
    if ((rc = mepa_macsec_control_frame_match_conf_set(meba_macsec_rollover_instance
                                                           ->phy_devices[req->port_no],
    ...
    for (uint8_t i = 0; i < 2; i++) {
        ...
        if (rollover_direction_egress) {
            if (cipher_suit == MACSEC_XPN_CIPHER_SUIT) {
                /* TX SA Create */
                if ((rc = mepa_macsec_tx_seca_set(meba_macsec_rollover_instance
                                                      ->phy_devices[req->port_no],
                                                  macsec_port, i, next_pn, TRUE, &sak, &ssci)) !=
                    MEPA_RC_OK) {
                ...
                /* TX SA Create */
                if ((rc = mepa_macsec_tx_sa_set(meba_macsec_rollover_instance
                                                    ->phy_devices[req->port_no],
                                                macsec_port, i, next_pn.pn, TRUE, &sak)) !=
                    MEPA_RC_OK) {
            ...
            if (cipher_suit == MACSEC_XPN_CIPHER_SUIT) {
                /* RX SA Create */
                if ((rc = mepa_macsec_rx_seca_set(meba_macsec_rollover_instance
                                                      ->phy_devices[req->port_no],
                                                  macsec_port, &sci, i, next_pn, &sak, &ssci)) !=
                    MEPA_RC_OK) {
                ...
                /* RX SA Create */
                if ((rc = mepa_macsec_rx_sa_set(meba_macsec_rollover_instance
                                                    ->phy_devices[req->port_no],
                                                macsec_port, &sci, i, next_pn.pn, &sak)) !=
                    MEPA_RC_OK) {
    ...
    if (rollover_direction_egress) {
        /* Activate TX SA, AN= 0 */
        if ((rc =
                 mepa_macsec_tx_sa_activate(meba_macsec_rollover_instance->phy_devices[req->port_no],
                                            macsec_port, 0)) != MEPA_RC_OK) {
        ...
        /* Activate RX SA,  AN = 0 */
        if ((rc =
                 mepa_macsec_rx_sa_activate(meba_macsec_rollover_instance->phy_devices[req->port_no],
                                            macsec_port, &sci, 0)) != MEPA_RC_OK) {
...
static int inline rx_mkpdu_packet_poll(mepa_port_no_t port_no,
                                       int           *an_to_activate,
                                       uint64_t      *packet_pn)
{
    uint8_t               frame[1600];
...
static inline int mepa_egress_key_rollover(mepa_device_t *dev, mepa_port_no_t port_no)
{
    ...
    if ((rc = mepa_macsec_tx_sa_counters_get(dev, macsec_port, macsec_tx_sa_an_activated,
                                             &counters)) != MEPA_RC_OK) {
    ...
    store_rollover_log(port_no, active_an, new_an, pn, MACSEC_ROLLOVER_TX,
    ...
    if (!sa_created[precreate]) {
        ...
        if (rollover_egress_cipher_suit == MACSEC_XPN_CIPHER_SUIT) {
            ...
            mepa_macsec_pkt_num_t next_pn = {.pn = 1, .xpn = 1};
            /* TX SA Create */
            if ((rc = mepa_macsec_tx_seca_set(dev, macsec_port, precreate, next_pn, TRUE, &sak,
                                              &ssci)) != MEPA_RC_OK) {
                T_E("[ERROR] Tx XPN SA creation failed on port %d\n", port_no + 1);
                return 1;
            }
...
static inline int mepa_ingress_key_rollover(mepa_device_t *dev, mepa_port_no_t port_no)
{
    ...
    mepa_macsec_port_t macsec_port = {
        .port_no = port_no, .port_id = MACSEC_SECY_ID, .service_id = 0};
    ...
    if (!received) {
        ...
        for (int an = 0; an < 4; an++) {
            if (rx_sa_created[an] && rx_delete_schedule_time[an] &&
                now >= rx_delete_schedule_time[an]) {
    ...
    if (!rx_sa_created[precreate]) {
        ...
        if (rollover_ingress_cipher_suit == MACSEC_XPN_CIPHER_SUIT) {
            ...
            if ((rc = mepa_macsec_rx_seca_set(dev, macsec_port, &sci, precreate, next_pn, &sak,
                                              &ssci)) != MEPA_RC_OK) {
                T_E("[ERROR] Rx XPN SA creation failed on port %d\n", port_no + 1);
                return 1;
            }
...
static void cli_cmd_macsec_rollover_log_print(cli_req_t *req)
{
    ...
    if (rollover_egress_config_port_cnt == 1) {
        ...
        if ((rc = mepa_macsec_tx_sa_counters_get(meba_macsec_rollover_instance
                                                     ->phy_devices[rollover_egress_port_no],
                                                 macsec_port, macsec_tx_sa_an_activated,
                                                 &counters)) != MEPA_RC_OK) {
    ...
    uint32_t idx =
        (rollover_log_write_idx + MAX_ROLLOVER_LOGS - rollover_log_count) % MAX_ROLLOVER_LOGS;
    ...
    for (uint32_t i = 0; i < rollover_log_count; i++) {
        ...
        cli_printf(
            "[%03u] Time=%llu.%03llu s | Port=%u | %s | AN %u -> %u | PN=%llu | OutPktsEnc=%s\n", i,
            seconds, millis, e->port_no, (e->dir == MACSEC_ROLLOVER_TX) ? "TX" : "RX", e->old_an,
            e->new_an, e->pn_value, encrypted_pkts_str);
    }
}
...
static void cli_cmd_macsec_rollover_demo_disable(cli_req_t *req)
{
    mepa_rc            rc;
    mepa_macsec_port_t macsec_port = {
        .port_no = rollover_egress_port_no, .port_id = MACSEC_SECY_ID, .service_id = 0};
    if (rollover_egress_config_port_cnt == 1) {
        if ((rc = mepa_macsec_secy_conf_del(meba_macsec_rollover_instance
                                                ->phy_devices[rollover_egress_port_no],
                                            macsec_port)) != MEPA_RC_OK) {
        ...
        mepa_macsec_init_t init_data_enable = {.enable = 0,
                                               ...
                                               .mac_conf.hmac.dis_length_validate = 0,
                                               .bypass = MEPA_MACSEC_INIT_BYPASS_NONE};
        if ((rc = mepa_macsec_init_set(meba_macsec_rollover_instance
                                           ->phy_devices[rollover_egress_port_no],
                                       &init_data_enable)) != MEPA_RC_OK) {
    ...
    if (rollover_ingress_config_port_cnt == 1) {
        ...
        if ((rc = mepa_macsec_init_get(meba_macsec_rollover_instance
                                           ->phy_devices[rollover_ingress_port_no],
                                       &init_get)) != MEPA_RC_OK) {
...
{
    if (rollover_egress_config_port_cnt == 1) {
        mepa_egress_key_rollover(meba_macsec_rollover_instance->phy_devices[rollover_egress_port_no],
                                 rollover_egress_port_no);
    }

    if (rollover_ingress_config_port_cnt == 1) {
        mepa_ingress_key_rollover(meba_macsec_rollover_instance
                                      ->phy_devices[rollover_ingress_port_no],
                                  rollover_ingress_port_no);
    }
    return;
...
static cli_parm_t cli_parm_table[] = {
    ...
    {
     "xpn",                 "Enable XPN Cipher Suit",
     CLI_PARM_FLAG_SET,             cli_cmd_parse_xpn_param,
     cli_cmd_macsec_rollover_demo, },
};
...
void mepa_demo_appl_macsec_rollover_demo(mscc_appl_init_t *init)
{
    ...
    case MSCC_INIT_CMD_POLL_FASTEST: macsec_rollover_poll(); break;
    default:                         break;
    }
}
