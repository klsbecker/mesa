...
static mscc_appl_trace_module_t trace_module = {.name = "phy_ts"};
...
static int cli_cmd_parse_tc_op(cli_req_t *req)
{
    ...
    if (ts_keyword.tc_op_parsed == 1) {
        ...
        switch (tc_op) {
        case 0: mreq->tc_op_mode = MEPA_TS_TC_OP_MODE_A; break;

        case 1: mreq->tc_op_mode = MEPA_TS_TC_OP_MODE_B; break;

        case 2: mreq->tc_op_mode = MEPA_TS_TC_OP_MODE_C; break;
...
static int cli_cmd_parse_tx_fifo(cli_req_t *req)
{
    ...
    if (ts_keyword.tx_fifo_parsed == 1) {
        ...
        switch (tx_fifo) {
        case 0: mreq->tx_fifo_mode = MEPA_TS_FIFO_MODE_NORMAL; break;

        case 1: mreq->tx_fifo_mode = MEPA_TS_FIFO_MODE_SPI; break;
...
static int cli_cmd_parse_clksrc(cli_req_t *req)
{
    ...
    if (ts_keyword.clk_src_parsed == 1) {
        ...
        switch (clk_src) {
        case 0: mreq->clk_src = MEPA_TS_CLOCK_SRC_INTERNAL; break;

        case 1: mreq->clk_src = MEPA_TS_CLOCK_SRC_FROM_RX_PORT0; break;

        case 2: mreq->clk_src = MEPA_TS_CLOCK_SRC_FROM_RX_PORT1; break;

        case 3: mreq->clk_src = MEPA_TS_CLOCK_SRC_FROM_RX_PORT2; break;

        case 4: mreq->clk_src = MEPA_TS_CLOCK_SRC_FROM_RX_PORT3; break;

        case 5: mreq->clk_src = MEPA_TS_CLOCK_SRC_EXTERNAL; break;
...
static int cli_cmd_parse_ltc_time(cli_req_t *req)
{
    ...
    if (ts_keyword.ltc_time_parsed == 1) {
        ...
        if (sscanf(req->cmd, "%hu:%u:%u:%hhu", &mreq->sec_high, &mreq->sec_low, &mreq->nanoseconds,
                   &mreq->picoseconds) != 4) {
            cli_printf("\n Invalid Format");
            return 0;
        }
...
const char *cli_ts_capable_txt(mesa_bool_t enabled) { return (enabled ? "Capable" : "NotCapable"); }
                                  ...
                                  const mepa_ts_fifo_status_t     status)
{
    ...
    cli_printf("\n");
    cli_printf(
        "PHY Fifo read: Packet : %d -> port_no %u, msg_type: %d, domain %d, seq %d, Source portId : 0x%x:%x:%x:%x:%x:%x:%x:%x:%x:%x\n",
        packet_count, (port_no + 1), sig->msg_type, sig->domain_num, sig->sequence_id,
        sig->src_port_identity[0], sig->src_port_identity[1], sig->src_port_identity[2],
        sig->src_port_identity[3], sig->src_port_identity[4], sig->src_port_identity[5],
        sig->src_port_identity[6], sig->src_port_identity[7], sig->src_port_identity[8],
        sig->src_port_identity[9]);
    ...
    if (sig->dmac_sig_supported == TRUE && is_ipv4_zero) {
        cli_printf("PHY Fifo read: Packet : %d -> DMAC : 0x%02x:0x%02x:0x%02x:0x%02x:0x%02x:0x%02x\n",
                   packet_count, sig->dmac_addr.addr[0], sig->dmac_addr.addr[1],
                   sig->dmac_addr.addr[2], sig->dmac_addr.addr[3], sig->dmac_addr.addr[4],
                   sig->dmac_addr.addr[5]);
    }

    if (sig->ipv4_sig_supported == TRUE && is_dmac_zero) {
        cli_printf("PHY Fifo read: Packet : %d -> Dest IPv4 : %d.%d.%d.%d\n", packet_count,
                   sig->dest_ipv4[0], sig->dest_ipv4[1], sig->dest_ipv4[2], sig->dest_ipv4[3]);
    }
    cli_printf(
        "PHY Fifo read: Packet : %d -> Dest IPv6(hex) : %02x%02x.%02x%02x.%02x%02x.%02x%02x.%02x%02x.%02x%02x.%02x%02x.%02x%02x\n",
        packet_count, sig->ipv6_dest_addr[15], sig->ipv6_dest_addr[14], sig->ipv6_dest_addr[13],
        sig->ipv6_dest_addr[12], sig->ipv6_dest_addr[11], sig->ipv6_dest_addr[10],
        sig->ipv6_dest_addr[9], sig->ipv6_dest_addr[8], sig->ipv6_dest_addr[7],
        sig->ipv6_dest_addr[6], sig->ipv6_dest_addr[5], sig->ipv6_dest_addr[4],
        sig->ipv6_dest_addr[3], sig->ipv6_dest_addr[2], sig->ipv6_dest_addr[1],
        sig->ipv6_dest_addr[0]);

    cli_printf(
        "PHY Fifo read: Packet : %d -> tx time:  Sec_Hi:%d, Sec_Low:%u, Nsec: %u, sub-Nsec %u\n",
        packet_count, fifo_ts->seconds.high, fifo_ts->seconds.low, fifo_ts->nanoseconds,
        fifo_ts->picoseconds);
}
...
static void cli_cmd_ts_port_state(cli_req_t *req)
{
    ...
    for (int iport = 0; iport < MAX_PRTS; iport++) {
        ...
        if ((rc = mepa_dev_check(meba_ts_instance, iport)) != MEPA_RC_OK) {
            status = TXT_NOT_APP;
            cli_printf("%-8u%s\n", port_no, cli_ts_state_txt(status));
            continue;
        }
...
static void cli_cmd_ts_conf_init(cli_req_t *req)
{
    ...
    for (int iport = 0; iport < MAX_PRTS; iport++) {
        ...
        if (MEPA_RC_OK ==
            mepa_ts_init_conf_set(meba_ts_instance->phy_devices[iport], &ts_init_conf)) {
...
const char *match_mode_names[] = {"MEPA_TS_MATCH_MODE_RANGE", "MEPA_TS_MATCH_MODE_VALUE"};
const char *clk_mode_names[] = {"MEPA_TS_PTP_CLOCK_MODE_NONE", "MEPA_TS_PTP_CLOCK_MODE_BC1STEP",
                                "MEPA_TS_PTP_CLOCK_MODE_BC2STEP", "MEPA_TS_PTP_CLOCK_MODE_TC1STEP",
                                "MEPA_TS_PTP_CLOCK_MODE_TC2STEP"};
const char *delaym_type_names[] = {"MEPA_TS_PTP_DELAYM_P2P", "MEPA_TS_PTP_DELAYM_E2E"};
const char *pkt_encap_names[] = {"MEPA_TS_ENCAP_NONE",
...
const char *mac_match_mode_names[] = {"MEPA_TS_ETH_ADDR_MATCH_ANY", "MEPA_TS_ETH_ADDR_MATCH_48BIT",
                                      "MEPA_TS_ETH_ADDR_MATCH_ANY_UNICAST",
                                      "MEPA_TS_ETH_ADDR_MATCH_ANY_MULTICAST"};
const char *mac_match_select_names[] = {"MEPA_TS_ETH_MATCH_NONE", "MEPA_TS_ETH_MATCH_DEST_ADDR",
                                        "MEPA_TS_ETH_MATCH_SRC_ADDR",
                                        "MEPA_TS_ETH_MATCH_SRC_OR_DEST"};
const char *ip_ver_names[] = {"MEPA_TS_IP_VER_4", "MEPA_TS_IP_VER_6"};
const char *ip_match_mode_names[] = {"MEPA_TS_IP_MATCH_NONE", "MEPA_TS_IP_MATCH_DEST",
                                     "MEPA_TS_IP_MATCH_SRC", "MEPA_TS_IP_MATCH_SRC_OR_DEST"};

// Enum string tables
const char *clk_freq_names[] = {"MEPA_TS_CLOCK_FREQ_25M",    "MEPA_TS_CLOCK_FREQ_125M",
                                "MEPA_TS_CLOCK_FREQ_15625M", "MEPA_TS_CLOCK_FREQ_200M",
                                "MEPA_TS_CLOCK_FREQ_250M",   "MEPA_TS_CLOCK_FREQ_500M",
                                "MEPA_TS_CLOCK_FREQ_MAX"};
const char *clk_src_names[] = {"MEPA_TS_CLOCK_SRC_EXTERNAL",
...
const char *rx_ts_pos_names[] = {"MEPA_TS_RX_TIMESTAMP_POS_IN_PTP",
                                 "MEPA_TS_RX_TIMESTAMP_POS_AT_END"};
const char *rx_ts_len_names[] = {"MEPA_TS_RX_TIMESTAMP_LEN_30BIT",
                                 "MEPA_TS_RX_TIMESTAMP_LEN_32BIT"};
const char *tx_fifo_mode_names[] = {"MEPA_TS_FIFO_MODE_NORMAL", "MEPA_TS_FIFO_MODE_SPI"};
const char *tx_ts_len_names[] = {"MEPA_TS_FIFO_TIMESTAMP_LEN_4BYTE",
                                 "MEPA_TS_FIFO_TIMESTAMP_LEN_10BYTE"};
const char *tc_op_mode_names[] = {"MEPA_TS_TC_OP_MODE_A", "MEPA_TS_TC_OP_MODE_B",
                                  "MEPA_TS_TC_OP_MODE_C"};
const char *ts_len_names[] = {"MEPA_MCH_TS_32NS_ONS", "MEPA_MCH_TS_28NS_4SNS",
                              "MEPA_MCH_TS_24NS_8SNS", "MEPA_MCH_TS_16NS_16SNS"};
    ...
    while (fgets(line, sizeof(line), f)) {
        ...
        *eq = '\0';
        ...
        if (strcmp(tag, "clk_freq") == 0) {
            int idx = parse_enum(value, clk_freq_names,
                                 sizeof(clk_freq_names) / sizeof(clk_freq_names[0]));
            if (idx >= 0) {
                conf->clk_freq = (mepa_ts_clock_freq_t)idx;
            }
        } else if (strcmp(tag, "clk_src") == 0) {
            int idx =
                parse_enum(value, clk_src_names, sizeof(clk_src_names) / sizeof(clk_src_names[0]));
            if (idx >= 0) {
                conf->clk_src = (mepa_ts_clock_src_t)idx;
            }
        } else if (strcmp(tag, "rx_ts_pos") == 0) {
            int idx = parse_enum(value, rx_ts_pos_names,
                                 sizeof(rx_ts_pos_names) / sizeof(rx_ts_pos_names[0]));
            if (idx >= 0) {
                conf->rx_ts_pos = (mepa_ts_rx_timestamp_pos_t)idx;
            }
        } else if (strcmp(tag, "rx_ts_len") == 0) {
            int idx = parse_enum(value, rx_ts_len_names,
                                 sizeof(rx_ts_len_names) / sizeof(rx_ts_len_names[0]));
            if (idx >= 0) {
                conf->rx_ts_len = (mepa_ts_rx_timestamp_len_t)idx;
            }
        } else if (strcmp(tag, "tx_fifo_mode") == 0) {
            int idx = parse_enum(value, tx_fifo_mode_names,
                                 sizeof(tx_fifo_mode_names) / sizeof(tx_fifo_mode_names[0]));
            if (idx >= 0) {
                conf->tx_fifo_mode = (mepa_ts_fifo_mode_t)idx;
            }
        } else if (strcmp(tag, "tx_ts_len") == 0) {
            int idx = parse_enum(value, tx_ts_len_names,
                                 sizeof(tx_ts_len_names) / sizeof(tx_ts_len_names[0]));
            if (idx >= 0) {
            ...
            int idx = parse_enum(value, tc_op_mode_names,
                                 sizeof(tc_op_mode_names) / sizeof(tc_op_mode_names[0]));
            if (idx >= 0) {
        ...
        // Nested mch_pch_conf fields
            ...
            int idx =
                parse_enum(value, ts_len_names, sizeof(ts_len_names) / sizeof(ts_len_names[0]));
            if (idx >= 0) {
...
int parse_ipv6_address(const char *input, uint32_t *addr)
{
    ...
    if (sscanf(input, "%hx:%hx:%hx:%hx:%hx:%hx:%hx:%hx", &segments[0], &segments[1], &segments[2],
               &segments[3], &segments[4], &segments[5], &segments[6], &segments[7]) == 8) {
...
{
    return sscanf(input, "%hhx:%hhx:%hhx:%hhx:%hhx:%hhx", &octets[0], &octets[1], &octets[2],
                  &octets[3], &octets[4], &octets[5]) == 6;
}

static void ethtype_value_update(uint16_t  encap_type,
                                 uint16_t  ip_ver,
                                 uint16_t  num_tag,
                                 uint16_t *eth1_ethtype,
                                 uint16_t *eth2_ethtype)
{
    switch (encap_type) {
...
static int update_ts_classifier_encap(mepa_ts_classifier_t *ts_classifier, uint16_t encap_type)
{
    ...
    if ((encap_type == MEPA_TS_ENCAP_ETH_ETH_PTP) || (encap_type == MEPA_TS_ENCAP_ETH_ETH_IP_PTP) ||
        (encap_type == MEPA_TS_ENCAP_ETH_MPLS_ETH_PTP) ||
    ...
    if ((encap_type == MEPA_TS_ENCAP_ETH_IP_PTP) || (encap_type == MEPA_TS_ENCAP_ETH_IP_IP_PTP) ||
        (encap_type == MEPA_TS_ENCAP_ETH_ETH_IP_PTP) ||
        (encap_type == MEPA_TS_ENCAP_ETH_MPLS_IP_PTP) ||
        (encap_type == MEPA_TS_ENCAP_ETH_MPLS_ETH_IP_PTP)) {
        ...
        if (ip_match == 0) {
            ...
            ts_classifier->ip_class_conf.ip_addr.ipv4.mask =
                0xFFFFFFFF; /* Match complete 32-bits of IP Address */
            memcpy(&ts_classifier->ip_class_conf.ip_addr.ipv6.mask, ipv6_mask,
                   sizeof(ipv6_mask)); /* Match complete 128-bits of IP Address */
            if (nheaderreq == 2) {
                ts_classifier->ip2_class_conf.ip_match_mode = MEPA_TS_IP_MATCH_DEST;
                ts_classifier->ip2_class_conf.ip_addr.ipv4.mask =
                    0xFFFFFFFF; /* Match complete 32-bits of IP Address */
                memcpy(&ts_classifier->ip2_class_conf.ip_addr.ipv6.mask, ipv6_mask,
                       sizeof(ipv6_mask));
            }
        } else if (ip_match == 2) {
        ...
        if (ip_ver == 1) { // ipv6
            ...
            const char *prompts[] = {"Enter First Layer IPv4 address (format - x.x.x.x): ",
                                     "Enter Second Layer IPv4 address (format - x.x.x.x): "};

            uint32_t *addresses[] = {&ts_classifier->ip_class_conf.ip_addr.ipv4.addr,
...
static void cli_cmd_ts_tx_class_conf(cli_req_t *req)
{
    ...
    for (int iport = 0; iport < MAX_PRTS; iport++) {
        ...
        if (MEPA_RC_OK == mepa_ts_tx_classifier_conf_set(meba_ts_instance->phy_devices[iport],
                                                         flow_index, &ts_classifier)) {
    ...
    while (fgets(line, sizeof(line), f)) {
        ...
        *eq = '\0';
        ...
        if (strcmp(tag, "enable") == 0) {
            ...
            int idx = parse_enum(value, pkt_encap_names,
                                 sizeof(pkt_encap_names) / sizeof(pkt_encap_names[0]));
            if (idx >= 0) {
        ...
        // eth_class_conf
            ...
            int idx = parse_enum(value, mac_match_mode_names,
                                 sizeof(mac_match_mode_names) / sizeof(mac_match_mode_names[0]));
            if (idx >= 0) {
                conf->eth_class_conf.mac_match_mode = (mepa_ts_mac_match_mode_t)idx;
            }
        } else if (strcmp(tag, "eth_class_conf.mac_match_select") == 0) {
            int idx =
                parse_enum(value, mac_match_select_names,
                           sizeof(mac_match_select_names) / sizeof(mac_match_select_names[0]));
            if (idx >= 0) {
            ...
            conf->eth_class_conf.vlan_conf.inner_tag.match.range.lower =
                (uint16_t)strtoul(value, NULL, 0);
        } else if (strcmp(tag, "eth_class_conf.vlan_conf.inner_tag.match.range.upper") == 0) {
            conf->eth_class_conf.vlan_conf.inner_tag.match.range.upper =
                (uint16_t)strtoul(value, NULL, 0);
        } else if (strcmp(tag, "eth_class_conf.vlan_conf.inner_tag.match.value.val") == 0) {
            conf->eth_class_conf.vlan_conf.inner_tag.match.value.val =
                (uint16_t)strtoul(value, NULL, 0);
        } else if (strcmp(tag, "eth_class_conf.vlan_conf.inner_tag.match.value.mask") == 0) {
            conf->eth_class_conf.vlan_conf.inner_tag.match.value.mask =
                (uint16_t)strtoul(value, NULL, 0);
        }
        // ip_class_conf
        else if (strcmp(tag, "ip_class_conf.ip_ver") == 0) {
            int idx =
                parse_enum(value, ip_ver_names, sizeof(ip_ver_names) / sizeof(ip_ver_names[0]));
            if (idx >= 0) {
                conf->ip_class_conf.ip_ver = (mepa_ts_ip_ver_t)idx;
                ip_class_conf_ipver = idx;
            }
        } else if (strcmp(tag, "ip_class_conf.ip_match_mode") == 0) {
            int idx = parse_enum(value, ip_match_mode_names,
                                 sizeof(ip_match_mode_names) / sizeof(ip_match_mode_names[0]));
            if (idx >= 0) {
                conf->ip_class_conf.ip_match_mode = (mepa_ts_ip_match_select_t)idx;
            }
        } else if (strncmp(tag, "ip_class_conf.ip_addr.ipv4.", 27) == 0 &&
                   ip_class_conf_ipver == 0) {
        ...
        } else if (strncmp(tag, "ip_class_conf.ip_addr.ipv6.", 27) == 0 &&
                   ip_class_conf_ipver == 1) {
        ...
        // eth2_class_conf
            ...
            if (strcmp(subtag, "mac_match_mode") == 0) {
                int idx =
                    parse_enum(value, mac_match_mode_names,
                               sizeof(mac_match_mode_names) / sizeof(mac_match_mode_names[0]));
                if (idx >= 0) {
                ...
                conf->eth2_class_conf.vlan_conf.outer_tag.match.range.lower =
                    (uint16_t)strtoul(value, NULL, 0);
            } else if (strcmp(subtag, "vlan_conf.outer_tag.match.range.upper") == 0) {
                conf->eth2_class_conf.vlan_conf.outer_tag.match.range.upper =
                    (uint16_t)strtoul(value, NULL, 0);
            } else if (strcmp(subtag, "vlan_conf.outer_tag.match.value.val") == 0) {
                conf->eth2_class_conf.vlan_conf.outer_tag.match.value.val =
                    (uint16_t)strtoul(value, NULL, 0);
            } else if (strcmp(subtag, "vlan_conf.outer_tag.match.value.mask") == 0) {
                conf->eth2_class_conf.vlan_conf.outer_tag.match.value.mask =
                    (uint16_t)strtoul(value, NULL, 0);
                ...
                conf->eth2_class_conf.vlan_conf.inner_tag.match.range.lower =
                    (uint16_t)strtoul(value, NULL, 0);
            } else if (strcmp(subtag, "vlan_conf.inner_tag.match.range.upper") == 0) {
                conf->eth2_class_conf.vlan_conf.inner_tag.match.range.upper =
                    (uint16_t)strtoul(value, NULL, 0);
            } else if (strcmp(subtag, "vlan_conf.inner_tag.match.value.val") == 0) {
                conf->eth2_class_conf.vlan_conf.inner_tag.match.value.val =
                    (uint16_t)strtoul(value, NULL, 0);
            } else if (strcmp(subtag, "vlan_conf.inner_tag.match.value.mask") == 0) {
                conf->eth2_class_conf.vlan_conf.inner_tag.match.value.mask =
                    (uint16_t)strtoul(value, NULL, 0);
            }
        }
        // ip2_class_conf
        ...
        } else if (strncmp(tag, "ip2_class_conf.ip_addr.ipv4.", 27) == 0 &&
                   ip2_class_conf_ipver == 0) {
        ...
        } else if (strncmp(tag, "ip2_class_conf.ip_addr.ipv6.", 27) == 0 &&
                   ip2_class_conf_ipver == 1) {
        ...
        // mpls_class_conf
            ...
            if (strcmp(subtag, "cw_en") == 0) {
                ...
                conf->mpls_class_conf.stack_level.first_lvl.lower =
                    (uint32_t)strtoul(value, NULL, 0);
            } else if (strcmp(subtag, "stack_level.first_lvl.upper") == 0) {
                conf->mpls_class_conf.stack_level.first_lvl.upper =
                    (uint32_t)strtoul(value, NULL, 0);
                ...
                conf->mpls_class_conf.stack_level.second_lvl.lower =
                    (uint32_t)strtoul(value, NULL, 0);
            } else if (strcmp(subtag, "stack_level.second_lvl.upper") == 0) {
                conf->mpls_class_conf.stack_level.second_lvl.upper =
                    (uint32_t)strtoul(value, NULL, 0);
                ...
                conf->mpls_class_conf.stack_level.third_lvl.lower =
                    (uint32_t)strtoul(value, NULL, 0);
            } else if (strcmp(subtag, "stack_level.third_lvl.upper") == 0) {
                conf->mpls_class_conf.stack_level.third_lvl.upper =
                    (uint32_t)strtoul(value, NULL, 0);
                ...
                conf->mpls_class_conf.stack_level.fourth_lvl.lower =
                    (uint32_t)strtoul(value, NULL, 0);
            } else if (strcmp(subtag, "stack_level.fourth_lvl.upper") == 0) {
                conf->mpls_class_conf.stack_level.fourth_lvl.upper =
                    (uint32_t)strtoul(value, NULL, 0);
...
static void cli_cmd_ts_tx_clock_conf(cli_req_t *req)
{
    ...
    for (int iport = 0; iport < MAX_PRTS; iport++) {
        ...
        if (mepa_ts_tx_clock_conf_get(meba_ts_instance->phy_devices[iport], clk_id, &ts_clock) !=
            MEPA_RC_OK) {
        ...
        if (MEPA_RC_OK ==
            mepa_ts_tx_clock_conf_set(meba_ts_instance->phy_devices[iport], clk_id, &ts_clock)) {
    ...
    while (fgets(line, sizeof(line), f)) {
        ...
        *eq = '\0';
        ...
        // ptp_class_conf.domain
            ...
            int idx = parse_enum(value, match_mode_names,
                                 sizeof(match_mode_names) / sizeof(match_mode_names[0]));
            if (idx >= 0) {
        ...
        // clk_mode
            ...
            int idx = parse_enum(value, clk_mode_names,
                                 sizeof(clk_mode_names) / sizeof(clk_mode_names[0]));
            if (idx >= 0) {
        ...
        // delaym_type
            ...
            int idx = parse_enum(value, delaym_type_names,
                                 sizeof(delaym_type_names) / sizeof(delaym_type_names[0]));
            if (idx >= 0) {
...
static void cli_cmd_ts_rx_class_conf(cli_req_t *req)
{
    ...
    for (int iport = 0; iport < MAX_PRTS; iport++) {
        ...
        ts_classifier.pkt_encap_type = mreq->encap_type;
        ts_classifier.enable = TRUE;
        if (MEPA_RC_OK == mepa_ts_rx_classifier_conf_set(meba_ts_instance->phy_devices[iport],
                                                         flow_index, &ts_classifier)) {
...
static void cli_cmd_ts_rx_clock_conf(cli_req_t *req)
{
    ...
    for (int iport = 0; iport < MAX_PRTS; iport++) {
        ...
        if (mepa_ts_rx_clock_conf_get(meba_ts_instance->phy_devices[iport], clk_id, &ts_clock) !=
            MEPA_RC_OK) {
        ...
        if (MEPA_RC_OK ==
            mepa_ts_rx_clock_conf_set(meba_ts_instance->phy_devices[iport], clk_id, &ts_clock)) {
...
static void cli_cmd_ts_ltc_ls(cli_req_t *req)
{
    ...
    case 2:  ls_type = MEPA_TS_CMD_DELTA; break;
    case 3:  ls_type = MEPA_TS_CMD_TOD; break;
    default: cli_printf("\n Pin Action not support on port : %d\n", (req->port_no + 1)); return;
    }
...
static void cli_cmd_ts_pps_incfg(cli_req_t *req)
{
    ...
    case 2: pin_conf.pin_sync_mode = LAN80XX_PTP_ACTION_CONTINUOUS; break;
    default:
    ...
    if (MEPA_RC_OK == lan80xx_phy_ts_pps_input_confset(meba_ts_instance->phy_devices[req->port_no],
...
static void cli_cmd_ts_1pps_out_en(cli_req_t *req)
{
    ...
    /* This Chip-specific API "lan80xx_phy_ts_pps_ouput_conf_set" has wider option to select
     * Syncronous/Asyncronous PPS signal, If User wants 1PPS similar to other MCHP PHYs user can use
     * MEPA API "mepa_ts_pps_conf_set" which generates Asyncronous 1PPS signal on LSC1 Pin
     * hardcoded,as there is no option to select PIN
     */
...
static void cli_cmd_ts_fifo_get(cli_req_t *req)
{
    ...
    if (status) {
        packet_count = 0;
        /*
         * This API "mepa_ts_fifo_empty" will read the FIFO for timestamp and PTP signature, and
         * provide it to callback function register through "mepa_ts_fifo_install()" API, in this
         * Application function "test_ts_phy_fifo_read" is registered as callback function which
         * will print the Timestamp and signature
         */
...
static void cli_cmd_ts_sig_conf(cli_req_t *req)
{
    ...
    case 1:
        phy_sig_mask = (MEPA_TS_PTP_FIFO_SIG_SEQ_ID | MEPA_TS_PTP_FIFO_SIG_MSG_TYPE |
                        MEPA_TS_PTP_FIFO_SIG_DOMAIN_NUM);
        break;
    case 2:
        phy_sig_mask = (MEPA_TS_PTP_FIFO_SIG_SEQ_ID | MEPA_TS_PTP_FIFO_SIG_MSG_TYPE |
                        MEPA_TS_PTP_FIFO_SIG_DOMAIN_NUM | MEPA_TS_PTP_FIFO_SIG_SOURCE_PORT_ID);
        break;
    case 3:
        phy_sig_mask = (MEPA_TS_PTP_FIFO_SIG_SEQ_ID | MEPA_TS_PTP_FIFO_SIG_MSG_TYPE |
                        MEPA_TS_PTP_FIFO_SIG_DOMAIN_NUM | MEPA_TS_PTP_FIFO_SIG_DEST_IP |
                        MEPA_TS_PTP_FIFO_SIG_SRC_IP);
        break;
    case 4:
        phy_sig_mask = (MEPA_TS_PTP_FIFO_SIG_SEQ_ID | MEPA_TS_PTP_FIFO_SIG_MSG_TYPE |
                        MEPA_TS_PTP_FIFO_SIG_DOMAIN_NUM | MEPA_TS_PTP_FIFO_SIG_DEST_IP |
                        MEPA_TS_PTP_FIFO_SIG_SRC_IP | MEPA_TS_PTP_FIFO_SIG_SOURCE_PORT_ID);
        break;
    case 5:
        phy_sig_mask = (MEPA_TS_PTP_FIFO_SIG_SEQ_ID | MEPA_TS_PTP_FIFO_SIG_MSG_TYPE |
                        MEPA_TS_PTP_FIFO_SIG_DOMAIN_NUM | MEPA_TS_PTP_FIFO_SIG_DEST_MAC);
        break;
    case 6:
        phy_sig_mask = (MEPA_TS_PTP_FIFO_SIG_SEQ_ID | MEPA_TS_PTP_FIFO_SIG_MSG_TYPE |
                        MEPA_TS_PTP_FIFO_SIG_DOMAIN_NUM | MEPA_TS_PTP_FIFO_SIG_DEST_IP |
                        MEPA_TS_PTP_FIFO_SIG_SRC_IP | MEPA_TS_PTP_FIFO_SIG_DEST_MAC);
        break;
    case 7:
        if (phy_family.family == PHY_FAMILY_MALIBU_25G) {
            phy_sig_mask = (MEPA_TS_PTP_FIFO_SIG_SEQ_ID | MEPA_TS_PTP_FIFO_SIG_MSG_TYPE |
                            MEPA_TS_PTP_FIFO_SIG_DOMAIN_NUM | MEPA_TS_PTP_FIFO_SIG_DEST_IP |
                            MEPA_TS_PTP_FIFO_SIG_SRC_IP | MEPA_TS_PTP_FIFO_SIG_IPV6_DEST_IP);
        } else {
            cli_printf("\n Option Not supported by PHY at port:%d \n", req->port_no);
            return;
        }
        break;
    default: cli_printf("\n Invalid Signal configuration selected "); return;
    }
...
static void cli_cmd_ts_delay_set(cli_req_t *req)
{
    ...
    if (mreq->delay_mode == 0) {
        ...
        if (MEPA_RC_OK ==
            mepa_ts_path_delay_set(meba_ts_instance->phy_devices[req->port_no], &delay)) {
        ...
        if (MEPA_RC_OK ==
            mepa_ts_egress_latency_set(meba_ts_instance->phy_devices[req->port_no], &delay)) {
        ...
        if (MEPA_RC_OK ==
            mepa_ts_ingress_latency_set(meba_ts_instance->phy_devices[req->port_no], &delay)) {
...
static void cli_cmd_ts_delay_get(cli_req_t *req)
{
    ...
    if (mreq->delay_mode == 0) {
        ...
        if (MEPA_RC_OK ==
            mepa_ts_path_delay_get(meba_ts_instance->phy_devices[req->port_no], &delay)) {
        ...
        if (MEPA_RC_OK ==
            mepa_ts_egress_latency_get(meba_ts_instance->phy_devices[req->port_no], &delay)) {
        ...
        if (MEPA_RC_OK ==
            mepa_ts_ingress_latency_get(meba_ts_instance->phy_devices[req->port_no], &delay)) {
...
static void cli_cmd_ts_flow_disable(cli_req_t *req)
{
    ...
    rc = mepa_ts_tx_clock_conf_get(meba_ts_instance->phy_devices[req->port_no], clock_id,
                                   &tx_clk_conf);
    if (rc != MEPA_RC_OK) {
    ...
    rc = mepa_ts_rx_classifier_conf_get(meba_ts_instance->phy_devices[req->port_no], flow_index,
                                        &rx_class_conf);
    if (rc != MEPA_RC_OK) {
        cli_printf("\n Failed to Get TX Classifier conf\n");
        return;
    ...
    rc = mepa_ts_rx_clock_conf_get(meba_ts_instance->phy_devices[req->port_no], clock_id,
                                   &rx_clk_conf);
    if (rc != MEPA_RC_OK) {
        cli_printf("\n Failed to Get RX Classifier conf\n");
        return;
    ...
    rc = mepa_ts_tx_classifier_conf_set(meba_ts_instance->phy_devices[req->port_no], flow_index,
                                        &tx_class_conf);
    if (rc != MEPA_RC_OK) {
    ...
    rc = mepa_ts_tx_clock_conf_set(meba_ts_instance->phy_devices[req->port_no], clock_id,
                                   &tx_clk_conf);
    if (rc != MEPA_RC_OK) {
    ...
    rc = mepa_ts_rx_classifier_conf_set(meba_ts_instance->phy_devices[req->port_no], flow_index,
                                        &rx_class_conf);
    if (rc != MEPA_RC_OK) {
        cli_printf("\n Failed to Set TX Classifier conf\n");
        return;
    ...
    rc = mepa_ts_rx_clock_conf_set(meba_ts_instance->phy_devices[req->port_no], clock_id,
                                   &rx_clk_conf);
    if (rc != MEPA_RC_OK) {
        cli_printf("\n Failed to Set RX Classifier conf\n");
        return;
...
static void cli_cmd_ts_conf_get(cli_req_t *req)
{
    ...
    if (mreq->config == 0) { // mepa_ts_init_conf_get
        ...
        cli_printf("\n---------------TS Init Configuration: Port %d-------------------------",
                   (req->port_no + 1));
    } else if (mreq->config == 1) {
        mepa_ts_classifier_t class_conf;
        if (mepa_ts_tx_classifier_conf_get(meba_ts_instance->phy_devices[req->port_no],
                                           flw_idx_clk_id, &class_conf) == 0) {
            json_rpc_new_mepa_ts_classifier_t(&json_req, &jobj, &class_conf);
        }
        cli_printf("\n---------------TS Tx Classifier Configuration: Port %d-------------------------",
                   (req->port_no + 1));
        cli_printf("\n flow_index: %x", flw_idx_clk_id);
    } else if (mreq->config == 2) {
        mepa_ts_ptp_clock_conf_t clk_conf;
        if (mepa_ts_tx_clock_conf_get(meba_ts_instance->phy_devices[req->port_no], flw_idx_clk_id,
                                      &clk_conf) == 0) {
            json_rpc_new_mepa_ts_ptp_clock_conf_t(&json_req, &jobj, &clk_conf);
        }
        cli_printf("\n---------------TS Tx Clock Configuration: Port %d-------------------------",
                   (req->port_no + 1));
        cli_printf("\n clock_id: %x", flw_idx_clk_id);
    } else if (mreq->config == 3) {
        mepa_ts_classifier_t class_conf;
        if (mepa_ts_rx_classifier_conf_get(meba_ts_instance->phy_devices[req->port_no],
                                           flw_idx_clk_id, &class_conf) == 0) {
            json_rpc_new_mepa_ts_classifier_t(&json_req, &jobj, &class_conf);
        }
        cli_printf("\n---------------TS Rx Classifier Configuration: Port %d-------------------------",
                   (req->port_no + 1));
        cli_printf("\n flow_index: %x", flw_idx_clk_id);
    } else if (mreq->config == 4) {
        mepa_ts_ptp_clock_conf_t clk_conf;
        if (mepa_ts_rx_clock_conf_get(meba_ts_instance->phy_devices[req->port_no], flw_idx_clk_id,
                                      &clk_conf) == 0) {
            json_rpc_new_mepa_ts_ptp_clock_conf_t(&json_req, &jobj, &clk_conf);
        }
        cli_printf("\n---------------TS Rx Clock Configuration: Port %d-------------------------",
                   (req->port_no + 1));
        cli_printf("\n clock_id: %x", flw_idx_clk_id);
...
//     if(MEPA_RC_OK == mepa_ts_clock_rateadj_set(meba_ts_instance->phy_devices[req->port_no],
//     &rate_adj)) {
...
//     if(MEPA_RC_OK == mepa_ts_clock_rateadj_get(meba_ts_instance->phy_devices[req->port_no],
//     &rate_adj)) {
...
static void cli_cmd_ts_cmds()
{
    ...
    cli_printf("\n %-20s| %-80s| %s", "ts_init_conf",
               " <port_list> clk_src <clk_src> tx_fifo <tx_fifo> tc_op <tc_op>",
               "Initialize TS Block");
    cli_printf("\n %-20s| %-80s| %s", "", " dly_10b <dly_10b> tx_af <tx_af> mch <mch>", "");
    cli_printf("\n %-20s| %-80s| %s", "tx_class_conf",
               " <port_list> eng_id <eng_idx> entype <entype>",
               "Configures Egress Port Classifier");
    cli_printf("\n %-20s| %-80s| %s", "tx_clock_conf",
               " <port_list> eng_id <eng_idx> clk_mode <clk_mode> dly_type <dly_type>",
               "Configures Egress Port Clock");
    cli_printf("\n %-20s| %-80s| %s", "rx_class_conf",
               " <port_list> eng_id <eng_idx> entype <entype>",
               "Configures Ingress Port Classifier");
    cli_printf("\n %-20s| %-80s| %s", "rx_clock_conf",
               " <port_list> eng_id <eng_idx> clk_mode <clk_mode> dly_type <dly_type>",
               "Configures Ingress Port Clock");
    ...
    cli_printf("\n %-20s| %-80s| %s", "ts_ltc_set", " <port_no> time <sh:sl:ns:ps>",
               "Set Local Time Counter");
    cli_printf("\n %-20s| %-80s| %s", "ts_pps_incfg",
               " <port_no> clk_sel <clk_sel> sync_mode <sync_mode>",
               "Configure LS controller Input pps mode");
    cli_printf("\n %-20s| %-80s| %s", "", "ls_ctrl_sel <ls_ctrl_sel>", "");
    cli_printf("\n %-20s| %-80s| %s", "ts_pps_outcfg",
               " <port_no> clk_sel <clk_sel> out_mode <out_mode_sel>",
               "Configure LS controller Output pps mode");
    cli_printf("\n %-20s| %-80s| %s", "",
               "pps_wid <pps_wid> pps_in <pps_in> ls_ctrl_sel <ls_ctrl_sel>", "");
    cli_printf("\n %-20s| %-80s| %s", "ts_sertod_incfg", " <port_no> pin_sel <pin_sel> pol <pol>",
               "Configure LS controller Input Serial ToD");
    cli_printf("\n %-20s| %-80s| %s", "",
               " sync_mode <sync_mode> ls_ctrl_sel <ls_ctrl_sel> [load|save]", "");
    cli_printf("\n %-20s| %-80s| %s", "ts_sertod_outcfg", " <port_no> pin_sel <pin_sel> pol <pol>",
               "Configure LS controller output Serial ToD");
    cli_printf("\n %-20s| %-80s| %s", "", " sync_mode <sync_mode> wfh <wfh> wfl <wfl>", "");
    cli_printf("\n %-20s| %-80s| %s", "ts_delta_adj",
               " <port_no> adj <ns:sns> ls_ctrl_sel <ls_ctrl_sel>",
               "Configure LTC ToD adjustment in ns, sns");
    cli_printf("\n %-20s| %-80s| %s", "ts_epps_conf",
               " <port_no> clk_sel <clk_sel> sync_mode <sync_mode>",
               "Configure LS Controller Input EPPS mode");
    cli_printf("\n %-20s| %-80s| %s", "", " ls_ctrl_sel <ls_ctrl_sel> det_cfg <det_cfg>", "");
    cli_printf("\n %-20s| %-80s| %s", "ts_ltc_ls", " <port_no> pin_action <pin_action>",
               "Select LTC Operation mode");
    cli_printf("\n %-20s| %-80s| %s", "ts_fifo_get_empty", " <port_no>",
               "Get and clear FIFO TS Entry");
    cli_printf("\n %-20s| %-80s| %s", "ts_delay_set",
               " <port_no> timing_mode <timing_mode> delay <delay>",
               "Set Time Interval/latency in ns");
    cli_printf("\n %-20s| %-80s| %s", "ts_delay_get", " <port_no> timing_mode <timing_mode>",
               "Get Time Interval/latency in ns");
    cli_printf("\n %-20s| %-80s| %s", "ts_port_state", "", "Provides TS State of all Ports");
    cli_printf("\n %-20s| %-80s| %s", "ts_conf_get", " <port_no> conf_sel <conf_sel>",
               "Get TS Configurations");
    cli_printf("\n %-20s| %-80s| %s", "ts_port_stati", " <port_no>", "Get TS Port Statistics");
    cli_printf("\n %-20s| %-80s| %s", "ts_1pps_out_en", " <port_no> ls_ctrl_sel <ls_ctrl_sel>",
               "Enable 1PPS output signal");
    cli_printf("\n %-20s| %-80s| %s", "ts_sig_set", "<port_no> sig <sig_val>",
               "PTP FIFO Signature Configuration");
    cli_printf("\n %-20s| %-80s| %s", "ts_flow_dis", " <port_no> eng_id <eng_idx>",
               "Disable TX, RX Classifier and Clock config on PTP Engine");
    cli_printf("\n %-20s| %-80s| %s", "ts_csr_rd", " <port_no> mmd <dev-id> csr_addr <csraddr>",
               "TS CSR Register Read Command");
    cli_printf("\n %-20s| %-80s| %s", "ts_csr_wr",
               " <port_no> mmd <dev-id> csr_addr <csraddr> csr_value <csrvalue>",
               "TS CSR Register Write Command");
    cli_printf("\n\n");
    return;
}
...
static cli_cmd_t cli_cmd_ts_table[] = {
    {
     "exit_ts",                                                                                          "Exit Time Stamping Demo Application",
     cli_cmd_ts_demo, },
    ...
    {
     "ts_port_state","Status of Time Stamping on all Ports",
     cli_cmd_ts_port_state, },

    {
     "ts_enable <port_list>",                                                                                                   "Enable the TS Block",
     cli_cmd_ts_ena, },

    {
     "ts_disable <port_list>",                                                                             "Disable the TS Block",
     cli_cmd_ts_dis, },

    {
     "ts_init_conf <port_list> clk_src <clk_src> tx_fifo <tx_fifo> tc_op <tc_op> dly_10b <dly_10b> tx_af <tx_af> mch <mch>",
     "Initialize TS Block",                                         cli_cmd_ts_conf_init,
     },

    {
     "tx_class_conf <port_list> eng_id <eng_idx> entype <entype>",                                                                                                 "Configures Tx classifier",
     cli_cmd_ts_tx_class_conf, },

    {
     "tx_clock_conf <port_list> eng_id <eng_idx> clk_mode <clk_mode> dly_type <dly_type>","Configures Tx clock",
     cli_cmd_ts_tx_clock_conf, },

    {
     "rx_class_conf <port_list> eng_id <eng_idx> entype <entype>",                                                                                                   "Configures Rx classifier",
     cli_cmd_ts_rx_class_conf, },

    {
     "rx_clock_conf <port_list> eng_id <eng_idx> clk_mode <clk_mode> dly_type <dly_type>",                                                                             "Configures Rx clock",
     cli_cmd_ts_rx_clock_conf, },

    {
     "ts_ltc_ls_en <port_no> pin_action <pin_action>",                                       "Configure Local Time Counter",
     cli_cmd_ts_ltc_ls, },

    {
     "ts_pps_incfg <port_no> clk_sel <clk_sel> sync_mode <sync_mode> ls_ctrl_sel <ls_ctrl_sel>",                                                                                                 "Configure LSC pin for LS controller Input",
     cli_cmd_ts_pps_incfg, },

    {
     "ts_1pps_out_en <port_no> ls_ctrl_sel <ls_ctrl_sel>",                                                                              " Enable 1PPS output on selected LSC Pin",
...

    {
     ...
     "Configure EPPS",                                                                                                                                                                       cli_cmd_ts_epps_config,
     },
    ...
    {
     "ts_fifo_get_empty <port_no>",                                               "Get FIFO entries",
     cli_cmd_ts_fifo_get, },

    {
     "ts_delay_set <port_no> timing_mode <timing_mode> delay <delay>",                                                                                                 "Set Time Interval",
     cli_cmd_ts_delay_set, },

    {
     "ts_delay_get <port_no> timing_mode <timing_mode>",                                                       "Get Time Interval",
    ...
    {
     "ts_sig_set <port_no> sig <sig_val>", "Configure TS FIFO Signature fields",
     cli_cmd_ts_sig_conf, },
    // {
    ...
    // },
    {
     "ts_reset <port_no>",                                                                                                  "Hard reset TS Block",
     cli_cmd_ts_reset, },
    {
     "ts_flow_dis <port_no> eng_id <eng_idx>","Disable Tx, RX Classifier and Clock configuration",
     cli_cmd_ts_flow_disable, },
    {
     "ts_csr_rd <port_no> mmd <dev-id> csr_addr <csraddr>",                                                                                                   "Read TS Block CSR Registers",
     cli_cmd_ts_csr_rd, },
    {
     "ts_csr_wr <port_no> mmd <dev-id> csr_addr <csraddr> csr_value <csrvalue>",                                                                           "Write TS Block CSR Registers",
     cli_cmd_ts_csr_wr, },
};
...
static cli_cmd_t cli_cmd_table[] = {
    {
     "PHY Ts", "Time Stamping Operational Demo",
     cli_cmd_ts_demo, },
};
...
static cli_parm_t cli_parm_table[] = {
    ...
    {
     "<clk_src>",                    "0 : Internal, 1 : Rx Port 0, 2 : Rx Port 1, 3 : Rx Port 2, 4 : Rx Port 3, 5 : External",
     CLI_PARM_FLAG_SET,                                                                                                                                        cli_cmd_parse_clksrc,

     },
    {
     "tx_fifo",    "Tx FIFO Mode",
     CLI_PARM_FLAG_NO_TXT,                                                                                                                                                  cli_cmd_parse_keyword,
     },
    {
     "<tx_fifo>",            "0 - Normal, 1 - SPI",
     CLI_PARM_FLAG_SET,                                                                                                                               cli_cmd_parse_tx_fifo,
    ...
    {
     "<tc_op>",    "0 - Mode A, 1 - Mode B, 2 - Mode C",
     CLI_PARM_FLAG_SET,                                                                                                                                                  cli_cmd_parse_tc_op,

     },
    {
     "dly_10b",              "Delay request message",
     CLI_PARM_FLAG_NO_TXT,                                                                                                                               cli_cmd_parse_keyword,
     },
    {
     "<dly_10b>",                    "0 - Disable, 1 - Store 10-byte ingress timestamp for delay request message",
     CLI_PARM_FLAG_NONE,                                                                                                                                          cli_cmd_parse_boolean,
     },
    {
     "tx_af",    "",
     CLI_PARM_FLAG_NO_TXT,                                                                                                                                                  cli_cmd_parse_keyword,
    ...
    {
     "<eng_idx>",                    "PTP Engine Number (Engine 0 or 1 or 2)",
    ...
    {
     "<entype>",                "0 : None, 1 : Eth PTP, 2 : Eth IP PTP, 3 : Eth IP IP PTP, 4 : Eth Eth PTP, 5 : Eth Eth IP PTP,\n \
        6 : Eth MPLS IP PTP, 7 : Eth MPLS Eth PTP, 8 : Eth MPLS Eth IP PTP",
     CLI_PARM_FLAG_SET,                                                                                                                               cli_cmd_parse_encap,
     },
    {
     "clk_mode",                    "",
     CLI_PARM_FLAG_NO_TXT,                                                                                                                                            cli_cmd_parse_keyword,
    ...
    {
     "<clk_mode>",   "0 - None, 1 - BC1STEP, 2 - BC2STEP, 3 - TC1STEP, 4 - TC2STEP",
     CLI_PARM_FLAG_NONE,                                                                                                                                                  cli_cmd_parse_u8_param,
     },
    {
     "dly_type",             "",
     CLI_PARM_FLAG_NO_TXT,                                                                                                                               cli_cmd_parse_keyword,
    ...
    {
     "<dly_type>",                    "0 - Peer-to-Peer, 1- End-to-End",
     CLI_PARM_FLAG_NONE,                                                                                                                                        cli_cmd_parse_u8_param,
     },
    {
    ...
    {
     "<pin_action>",                    "0 - Load, 1 - Save, 2 - Delta,  3 - Time of Day",
     CLI_PARM_FLAG_NONE,                                                                                                                                         cli_cmd_parse_u8_param,
     },
    {
     "ls_ctrl_sel",    "",
    ...
    {
     "<ns_en>",   "Nanosecond bitout Enable",
     CLI_PARM_FLAG_NONE,                                                                                                                                                  cli_cmd_parse_boolean,
     },
    {
     "pps_wid",           "",
     ...
     },
    {
     ...
     CLI_PARM_FLAG_NONE,                                                                                                                                       cli_cmd_parse_u32_param,
     },
    {
     "out_mode",   "",
     CLI_PARM_FLAG_NO_TXT,                                                                                                                                                  cli_cmd_parse_keyword,
    ...
    {
     "<ns:sns>", "ns - nanoseconds, sns - sub nanoseconds\n",
     CLI_PARM_FLAG_NONE,                                                                                                                                                  cli_cmd_parse_ltc_time,
     },
    {
     "time",            "",
     CLI_PARM_FLAG_NO_TXT,                                                                                                                                 cli_cmd_parse_keyword,
    ...
    {
     "<sh:sl:ns:ps>",                    "sh - seconds high, sl - seconds low, ns - nanoseconds, ps - picoseconds\n",
     CLI_PARM_FLAG_NONE,                                                                                                                                          cli_cmd_parse_ltc_time,
     },
    {
     "sig", "",
    ...
    {
     "<timing_mode>", "0 - Delay Asymmetry, 1 - Path Delay, 2 - Egress Latency, 3 - Ingress Latency\n",
     CLI_PARM_FLAG_NONE,                                                                                                                                                  cli_cmd_parse_u8_param,
     },
    {
     "delay",     "",
    ...
    {
     "<delay>",                    "Time Interval in ns\n",
     CLI_PARM_FLAG_NONE,                                                                                                                                           cli_cmd_parse_floating_point,
     },
    {
     "conf_sel", "",
     CLI_PARM_FLAG_NO_TXT,                                                                                                                                                  cli_cmd_parse_keyword,
    ...
    {
     "<conf_sel>",           "0 - Init Config, 1 - Tx Classifier Config, 2 - Tx Clock Config, \n \
    ...
    {
     "<det_cfg>", "ePPS event cycle detection adjustment",
     CLI_PARM_FLAG_NONE,                                                                                                                                                  cli_cmd_parse_u8_param,
     },
    {
     "rateadj",              "",
    ...
    {
     "<rateadj>",                    "Clock Rate adjust in ppb",
     CLI_PARM_FLAG_NONE,                                                                                                                                              cli_cmd_parse_floating_point,
     },
    {
     "clk_sel", "",
    ...
    {
     "<pin_sel>", "0 - PTP_LSC_PIN[0], 1 - PTP_LSC_PIN[1], 2 - PTP_LSC_PIN[2], 3 - PTP_LSC_PIN[3]",
     CLI_PARM_FLAG_NONE,                                                                                                                                                  cli_cmd_parse_u8_param,
     },
    {
     "pol",           "",
    ...
    {
     "mmd",                    "MMD ID for TS Block Registers",
     CLI_PARM_FLAG_NO_TXT,                                                                                                                                              cli_cmd_parse_keyword,
     },
    {
     "<dev-id>", "",
    ...
    {
     "csr_addr",          "CSR Register Address for TS Block",
     CLI_PARM_FLAG_NO_TXT,                                                                                                                                 cli_cmd_parse_keyword,
     },
    {
     "<csraddr>",                    "",
