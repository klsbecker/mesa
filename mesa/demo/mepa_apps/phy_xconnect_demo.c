...
static mscc_appl_trace_module_t trace_module = {.name = "phy_port_xconnect"};
...
static int get_port_count(uint8_t             port_no,
    ...
    if (phy_family.family == PHY_FAMILY_MALIBU_25G) {
        if (lan80xx_xconnect_conf_get(dev, req->port_no, &conf) != MESA_RC_OK) {
            ...
            if (conf.failover_conf.enable) {
                cli_printf("Mode :	%s\n",
                           (conf.failover_conf.mode == LAN80XX_AUTO_FAILOVER_1_ISTO_1_PROTECTION)
                               ? "1_ISTO_1_Hostprotection"
                               : "1_PLUS_1_Hostprotection");
            } else {
                cli_printf("Mode :	%s\n", "AUTOFAILOVER_PROTECTION_NONE");
            }
            cli_printf("Role :	%s\n", (conf.failover_conf.role == LAN80XX_WPS_PHY_ROLE)
                                           ? "PHY_ROLE"
                                           : "SYSTEM_ROLE");
            ...
            cli_printf("Active Host :	%s\n",
                       (conf.failover_conf.active_host == LAN80XX_WPS_0_1_DEFAULT_ACTIVE_H0_H3)
                           ? "H0_H3_activehost"
                           : "H1_H2_activehost");
...
static int cli_parm_value_port(cli_req_t *req)
{
    ...
    int ret = cli_parse_values(req->cmd, mreq->lineport_list, &mreq->value_cnt, mreq->min_port, 4,
                               mreq->max_port_cnt);
...
static void cli_cmd_xconnect_anylinetoanyhost(cli_req_t *req)
{
    ...
    if (phy_family.family == PHY_FAMILY_MALIBU_25G) {
        ...
        for (iport = 0; iport < mreq->max_port_cnt; iport++) {
            if ((meba_phy_conf_get(meba_xc_phy_instance, (mreq->lineport_list[iport] - 1),
                                   &config)) != MESA_RC_OK) {
        ...
        if (lan80xx_xconnect_anylinetoanyhost(dev, req->port_no, req->enable,
                                              mreq->lineport_channel_list) != MESA_RC_OK) {
...
static void cli_cmd_XconnectFailover(cli_req_t *req)
{
    ...
    snprintf(port_xconnect_file, sizeof(port_xconnect_file), "/root/mepa_scripts/%s",
             req->file_name);
    ...
    if (!(fseek(fp, 0, SEEK_END)) && ftell(fp)) {
        ...
        if (phy_family.family == PHY_FAMILY_MALIBU_10G) {
            if (json_rpc_get_name_json_object(&json_req, jobj, "phy_m10g", &json_req.params) !=
                MESA_RC_OK) {
                T_E("port_xconnect object name not found in file: %s", req->file_name);
                goto file_close;
            }
            if (json_rpc_get_vtss_phy_10g_auto_failover_conf_t(&json_req, json_req.params, &conf) !=
                MESA_RC_OK) {
            ...
            if (json_rpc_get_name_json_object(&json_req, jobj, "phy_m25g", &json_req.params) !=
                MESA_RC_OK) {
                T_E("port_xconnect object name not found in file: %s", req->file_name);
                goto file_close;
            }
            if (json_rpc_get_phy25g_autofailover_t(&json_req, json_req.params, &xconnect_conf) !=
                MESA_RC_OK) {
                T_E("Error in the json configuration");
                goto file_close;
            }
            if (lan80xx_xconnect_hostfailover_Protection(dev, req->port_no, &xconnect_conf) !=
                MESA_RC_OK) {
...
static cli_parm_t cli_parm_table[] = {
    ...
    {
     "<lineport_list>", "lineport_list   : List of Lineport numbers maps to Hostport 1,2,3,4 \n",
     CLI_PARM_FLAG_SET, cli_parm_value_port,
     }
};

static cli_cmd_t cli_cmd_table[] = {
    {"PHY Xconnect_failover <port_no> [-f] [filename]",
     "Set Cross connect failover for the Particular Port",                                                                          cli_cmd_XconnectFailover              },
    {"PHY CrossConnect_AnyLineToAnyHost <port_no> [enable|disable] <lineport_list>",
     "Configure any hostports to user defined line ports",                                                                          cli_cmd_xconnect_anylinetoanyhost     },
    {"PHY CrossConnect_failover_recovery  <port_no>",
     "Xconnect Failover recovery for connection fault detected port",                                                               cli_cmd_xconnect_hostfailover_recovery},
    {"PHY CrossConnect_force_failover  <port_no>",                                   "Xconnect Force failover for particular port",
     cli_cmd_xconnect_force_failover                                                                                                                                      },
    {"PHY Xconnect_conf_get  <port_no>",                                             "Get Xconnect for particular port",
