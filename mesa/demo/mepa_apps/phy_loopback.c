...
static mscc_appl_trace_module_t trace_module = {.name = "loopback"};
...
static int validate_loopback_config(mepa_loopback_t   conf,
                                    ...
                                    demo_phy_info_t   phy_family)
{
    ...
    mepa_loopback = ((conf.far_end_ena << 0) | (conf.near_end_ena << 1) |
                     (conf.connector_ena << 2) | (conf.mac_serdes_input_ena << 3) |
                     (conf.mac_serdes_facility_ena << 4) | (conf.mac_serdes_equip_ena << 5) |
                     (conf.media_serdes_input_ena << 6) | (conf.media_serdes_facility_ena << 7) |
                     (conf.media_serdes_equip_ena << 8) | (conf.qsgmii_pcs_tbi_ena << 9) |
    ...
    if (phy_family.family == PHY_FAMILY_MALIBU_25G) {
        chipspecific_loopback = ((loopback_25g.l3p_lp_ena << 0) | (loopback_25g.h3m_lp_ena << 1) |
                                 (loopback_25g.l3m_lp_ena << 2) | (loopback_25g.h7_lp_ena << 4));
...
static void cli_cmd_loopback_conf(cli_req_t *req)
{
    ...
    if (dev != NULL) {
        ...
        snprintf(loopback_config_file, sizeof(loopback_config_file), "/root/mepa_scripts/%s",
                 req->file_name);
        FILE *fp = fopen(loopback_config_file, "r");
        if (fp == NULL) {
            T_E("%s : file open error. Please provide Valid JSON file: loopback_config.json",
                req->file_name);
            return;
        }
        if (!(fseek(fp, 0, SEEK_END)) && ftell(fp)) {
            ...
            if ((phy_family.family == PHY_FAMILY_LAN8814) ||
                (phy_family.family == PHY_FAMILY_VIPER)) {
                if (json_rpc_get_name_json_object(&json_req, jobj, "1g_loopback",
                                                  &json_req.params) != MESA_RC_OK) {
                ...
                if ((validate_loopback_config(conf, loopback_25g, phy_family)) == 0) {
                    if (meba_phy_loopback_set(meba_lp_phy_inst, req->port_no, &conf) !=
                        MESA_RC_OK) {
                ...
                if (json_rpc_get_name_json_object(&json_req, jobj, "10g_loopback",
                                                  &json_req.params) != MESA_RC_OK) {
                    T_E("object name not found in file: %s", req->file_name);
                    goto file_close;
                }
                if (json_rpc_get_vtss_phy_10g_loopback_t(&json_req, json_req.params, &loopback) !=
                    MESA_RC_OK) {
                    T_E("Error in the json configuration");
                    goto file_close;
                }
                if (vtss_phy_10g_loopback_get(data->vtss_instance, req->port_no, &curr_loopback) !=
                    MESA_RC_OK) {
                    T_E("Error in reading the LOOPBACK configuration");
                    goto file_close;
                }
                curr_loopback.enable = 0;
                if (vtss_phy_10g_loopback_set(data->vtss_instance, req->port_no, &curr_loopback) !=
                    MESA_RC_OK) {
                    T_E("Error in disabling the current LOOPBACK");
                    goto file_close;
                }
                if (vtss_phy_10g_loopback_set(data->vtss_instance, req->port_no, &loopback) !=
                    MESA_RC_OK) {
                    T_E("Error in Configuring the LOOPBACK");
                }
                goto file_close;
                ...
                if (json_rpc_get_name_json_object(&json_req, jobj, "25g_loopback",
                                                  &json_req.params) != MESA_RC_OK) {
                    T_E("object name not found in file: %s", req->file_name);
                    goto file_close;
                }
                ...
                if (json_rpc_get_name_json_object(&json_req, jobj_25g, "mepa_loopback",
                                                  &json_req.params) != MESA_RC_OK) {
                    T_E("object name not found in file: %s", req->file_name);
                    goto file_close;
                }
                ...
                if (json_rpc_get_name_json_object(&json_req, jobj_25g, "chipspecific_loopback",
                                                  &json_req.params) != MESA_RC_OK) {
                    T_E("object name not found in file: %s", req->file_name);
                    goto file_close;
                }

                if (json_rpc_get_phy25g_lp_types_t(&json_req, json_req.params, &loopback_25g) !=
                    MESA_RC_OK) {
                    T_E("Error in the json configuration");
                    goto file_close;
                }
                if ((validate_loopback_config(conf, loopback_25g, phy_family)) == 0) {
                    ...
                    if (lan80xx_phy_loopback_conf_set(dev, req->port_no, &loopback_25g) !=
                        MESA_RC_OK) {
...
static cli_cmd_t cli_cmd_table[] = {
    {"PHY Loopback <port_no> [-f] [filename]", "Configure loopback for particular port number",
