...
static mscc_appl_trace_module_t trace_module = {.name = "phy_kr_aneg"};
...
static mepa_rc Islinkup(int port_no, mepa_adv_side_t port_type, u8 *speed)
{
    ...
    if ((rc = lan80xx_phy_csr_read(meba_phy_kr_inst->phy_devices[port_no], port_no, mmd, 0x01,
                                   &value)) != MEPA_RC_OK) {
    ...
    if ((rc = lan80xx_phy_csr_read(meba_phy_kr_inst->phy_devices[port_no], port_no, mmd, 0x8032,
                                   &value)) != MEPA_RC_OK) {
        T_E("\n Error Reading a Register on port : %d \n", (port_no));
        return MESA_RC_ERROR;
    }
    ...
    for (int iport = 0; iport < meba_phy_kr_inst->phy_device_cnt - 1; iport++) {
        ...
        if (req->set) {
            if (phy_family.family == PHY_FAMILY_MALIBU_25G) {
                ...
                if ((rc = mepa_conf_get(meba_phy_kr_inst->phy_devices[iport], &conf)) !=
                    MESA_RC_OK) {
                ...
                /* set new config */
                if ((rc = mepa_conf_set(meba_phy_kr_inst->phy_devices[iport], &conf)) !=
                    MESA_RC_OK) {
...
static void phy_kr_log_enable_disable(cli_req_t *req)
{
    ...
    for (int iport = 0; iport < meba_phy_kr_inst->phy_device_cnt - 1; iport++) {
        ...
        if ((rc = lan80xx_KRLog_Enable(meba_phy_kr_inst->phy_devices[iport], mreq->krlog_enable,
                                       mreq->line_port, mreq->host_port)) != MEPA_RC_OK) {
            if (mreq->line_port) {
...
char *getTap(uint32_t u32LD_TAPRecieved)
{
    ...
    case eKR_COEF_CM1:  return "CM1";
    case eKR_COEF_C0:   return "C0";
    case eKR_COEF_CP1:  return "CP1";
    case eKR_COEF_INIT: return "INIT";
...
char *getStatusReportSentToLP(uint32_t u32StatusReportSentToLP)
{
    ...
    case KR_COEF_NOT_UPDATED: return "KRCNU";
    case KR_COEF_UPDATED:     return "KRCU";
    case KR_COEF_MINIMUM:     return "KRCM";
    case KR_COEF_MAXIMUM:     return "KRCX ";
...
char *getCommandReceived(KR_Log *log)
{
    ...
    case eKR_COEF_INCR: return "INCR";
    case eKR_COEF_DECR: return "DECR";
    case eKR_COEF_HOLD: return "HOLD";
    default:            return "UNKNOWN";
    }
}
...
{
    switch (u32BER_STATE) {
    case BER_GO_TO_MIN:        return "BGT_MIN";
    case BER_CALCULATE_BER:    return "BER_CAL";
    case BER_MOVE_TO_MID_MARK: return "BMT_MM";
    default:                   return "BLRXT";
    }
}
void LogEntryAll(FILE *file, int rowNo, KR_Log *log, unsigned int delta)
{
    ...
    fprintf(
        file,
        "%-5d %-10u %-10d %-30s %-30s %-10s %-10s %-10d %-10d %-10d %-20s %-20s %-20u %-20u %-20u %-20u\n",
        rowNo, log->timestamp, delta, irqStr ? irqStr : "-", enumStr ? enumStr : "-",
        getTap(log->LD_TAPRecieved), getCommandReceived(log), log->CM1, log->C0, log->CP1,
        getCommandBER(log->BER_STATE), getStatusReportSentToLP(log->StatusReportSentToLP),
        log->EYE_HIGHT, log->CommandSentToLP, log->TAPSentToLP, log->StatusReportReceivedFromLP);
...
void LogEntryBER(FILE *file, int rowNo, KR_Log *log)
{
    ...
    // Print to console
    cli_printf("%-5d %-20s %-20s %-20s %-20s %-20u %-20s\n", rowNo, getTap(log->LD_TAPRecieved),
               getStatusReportSentToLP(log->StatusReportSentToLP), getCommandReceived(log),
               getCommandBER(log->BER_STATE), log->timestamp, irqStr ? irqStr : "-");
...
void LogEntryIRQ(FILE *file, int rowNo, KR_Log *log, unsigned int delta)
{
    ...
    // Print to console
    cli_printf("%-5d %-20u %-20d %-30s %-20s\n", rowNo, log->timestamp, delta,
               irqStr ? irqStr : "-", enumStr ? enumStr : "-");
...
void LogEntryEQ(FILE *file, int rowNo, KR_Log *log)
{
    ...
    cli_printf("%-5d %-20s %-20s %-20d %-20d %-20d %-20s %-20u\n", rowNo,
               getTap(log->LD_TAPRecieved), getCommandReceived(log), log->CM1, log->C0, log->CP1,
               getStatusReportSentToLP(log->StatusReportSentToLP), log->timestamp);
...
void LogEntryANEG(FILE *file, int rowNo, ANEG_Log *log, unsigned int delta)
{
    ...
    cli_printf("%-3d %-10u %-10u %-20s %-20s %-10s %-10s %-10s %-10s %-10s %-10s\n", rowNo,
               log->timestamp, delta, irqStr ? irqStr : "-", enumStr ? enumStr : "-",
               DecodeBasePage0(log->LPage_BP0), DecodeBasePage1(log->LPage_BP1),
               DecodeBasePage2(log->LPage_BP2), DecodeNextPage1(log->LPage_NP0),
               DecodeNextPage2(log->LPage_NP1), DecodeNextPage2(log->LPage_NP2));
...
*/
void processKrLogging(struct mepa_device *dev,
                      phy_kr_log_sel_t   *mreq,
                      mepa_port_no_t      port_no,
                      uint8_t             u8Line_host,
                      uint32_t            u32MemReadAddress,
                      uint16_t            u16DataLength)
{
    ...
    if (mreq->eq) {
        snprintf(szFileNamePortKRStatus, sizeof(szFileNamePortKRStatus),
                 "/root/mepa_scripts/Port_KR_Status_EQ_%s_PORT%d.log", side, port_no);
    } else if (mreq->ber) {
        snprintf(szFileNamePortKRStatus, sizeof(szFileNamePortKRStatus),
                 "/root/mepa_scripts/Port_KR_Status_BER_%s_PORT%d.log", side, port_no);
    } else if (mreq->irq) {
        snprintf(szFileNamePortKRStatus, sizeof(szFileNamePortKRStatus),
                 "/root/mepa_scripts/Port_KR_Status_IRQ_%s_PORT%d.log", side, port_no);
    } else if (mreq->all) {
        snprintf(szFileNamePortKRStatus, sizeof(szFileNamePortKRStatus),
                 "/root/mepa_scripts/Port_KR_Status_Summary_%s_PORT%d.log", side, port_no);
        snprintf(szFileNamePortKRStatusAll, sizeof(szFileNamePortKRStatusAll),
                 "/root/mepa_scripts/Port_KR_Status_All_%s_PORT%d.log", side, port_no);
    } else if (mreq->aneg) {
        snprintf(szFileNamePortKRStatus, sizeof(szFileNamePortKRStatus),
                 "/root/mepa_scripts/Port_KR_Status_ANEG_%s_PORT%d.log", side, port_no);
    }
    ...
    snprintf(szFileNamePortKRLogMemoryDump, sizeof(szFileNamePortKRLogMemoryDump),
             "/root/mepa_scripts/Port_KR_Log_Memory_Dump_%s_PORT%d.log", side, port_no);
    ...
    SaveMemoryBytesToFile(fileHandlePortKRLogMemoryDump, pu8DataBuffer, u16DataLength,
                          u32MemReadAddress);
    ...
    if (mreq->aneg) {
        for (int j = 0; j < (u16DataLength);) {
            ...
            if (!bHeaderUpdate) {
                ...
                cli_printf("\n%-3s %-10s %-10s %-20s %-20s %-10s %-10s %-10s %-10s %-10s %-10s\n",
                           "S.No", "TimeStamp", "Delta", "IRQ", "Statemachine", "LP_BP0", "LP_BP1",
                           "LP_BP2", "LP_NP0", "LP_NP1", "LP_NP2");
                cli_printf(
                    "-----------------------------------------------------------------------------------------------------------------------------------\n");
        ...
        for (int j = 0; j < (u16DataLength);) {
            ...
            } else if ((asKRLog.LD_TAPRecieved == eKR_COEF_CP1) &&
                       (asKRLog.IRQ & IRQ_VEC1_LPCVALID)) {
                aEYEHTCP1[iye++] = asKRLog.EYE_HIGHT;
            } else if ((asKRLog.LD_TAPRecieved == eKR_COEF_C0) &&
                       (asKRLog.IRQ & IRQ_VEC1_LPCVALID)) {
            ...
            if (mreq->ber) {
                if (!bHeaderUpdate) {
                    ...
                    cli_printf("\n%-5s %-20s %-20s %-20s %-20s %-20s %-20s\n", "S.No", "RxTAP",
                               "RxLPS", "TxLPC", "BER State", "Timestamp", "IRQ's");
                    cli_printf(
                        "----------------------------------------------------------------------------------------------------------------------------\n");
            ...
            if (mreq->eq) {
                if (!bHeaderUpdate) {
                    ...
                    cli_printf("\n%-5s %-20s %-20s %-20s %-20s %-20s %-20s %-20s\n", "S.No",
                               "TAP(LP)", "CMD(LP)", "CM1(LD)", "Ampl(LD)", "CP1(LD)", "Status(LD)",
                               "TimeStamp");
                    cli_printf(
                        "----------------------------------------------------------------------------------------------------------------------------------------------\n");
            ...
            if (mreq->all) {
                if (!bHeaderUpdate) {
                    ...
                    fprintf(
                        fileHandlePortKRAll,
                        "BER State: BGT_MIN - BER_GO_TO_MIN, BER_CALC - BER_CALCULATE_BER, BMT_MM - BER_MOVE_TO_MID_MARK, BLRXT - BER_LOCAL_RX_TRAINED\n");
                    fprintf(
                        fileHandlePortKRAll,
                        "Rx LPS: KRCNU - KR_COEF_NOT_UPDATED, KRCU - KR_COEF_UPDATED, KRCM - KR_COEF_MINIMUM, KRCX - KR_COEF_MAXIMUM\n");
                    ...
                    fprintf(
                        fileHandlePortKRAll,
                        "%-5s %-10s %-10s %-30s %-30s %-10s %-10s %-10s %-10s %-10s %-20s %-20s %-20s %-20s %-20s %-20s\n",
                        "S.No", "TimeStamp", "Delta", "IRQ", "Statemachine", "TAP", "Tx LPC", "CM1",
                        "CM0", "CP1", "BER State", "Rx LPS", "Eye Height", "CMD Sent To LP",
                        "TAP Sent To LP", "Status Received from LP");
                    fprintf(
                        fileHandlePortKRAll,
                        "----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------\n");
    ...
    if (mreq->all) {
        ...
        asKRSummaryLog.FEC_CORRECTION =
            MAKEDWORD(dwPCS25G_FEC74_CERR_CNT_L, dwPCS25G_FEC74_CERR_CNT_H);
        ...
        asKRSummaryLog.FEC_UNCORRECTION =
            MAKEDWORD(dwPCS25G_FEC74_NCERR_CNT_L, dwPCS25G_FEC74_NCERR_CNT_H);
        ...
        sprintf(
            ...
            : ((asKRSummaryLog.wAneg_status & 0x08) == 0x08) ? "25G"
                                                             : "Undefined",
            ((asKRSummaryLog.wAneg_status & 0x20) == 0x20) ? "Enabled" : "Disabled",
            ((asKRSummaryLog.wAneg_status & 0x40) == 0x40) ? "Enabled" : "Disabled",
            asKRSummaryLog.LP_CM1_MAX, asKRSummaryLog.LP_CM1_END, asKRSummaryLog.LP_C0_MAX,
            asKRSummaryLog.LP_C0_END, asKRSummaryLog.LP_CP1_MAX, asKRSummaryLog.LP_CP1_END,
            asKRSummaryLog.EYE_HT_CM1[0], asKRSummaryLog.EYE_HT_CM1[1],
            asKRSummaryLog.EYE_HT_CM1[2], asKRSummaryLog.EYE_HT_CM1[3],
            asKRSummaryLog.EYE_HT_CM1[4], asKRSummaryLog.EYE_HT_CM1[5],
            asKRSummaryLog.EYE_HT_CM1[6], asKRSummaryLog.EYE_HT_CM1[7],
            asKRSummaryLog.EYE_HT_CM1[8], asKRSummaryLog.EYE_HT_CM1[9],
            asKRSummaryLog.EYE_HT_CM1[10], asKRSummaryLog.EYE_HT_CM1[11],
            asKRSummaryLog.EYE_HT_CM1[12], asKRSummaryLog.EYE_HT_CM1[13],
            asKRSummaryLog.EYE_HT_CM1[14], asKRSummaryLog.EYE_HT_CM1[15],
            asKRSummaryLog.EYE_HT_CM1[16], asKRSummaryLog.EYE_HT_CM1[17],
            asKRSummaryLog.EYE_HT_CM1[18], asKRSummaryLog.EYE_HT_CM1[19],
            asKRSummaryLog.EYE_HT_CM1[20], asKRSummaryLog.EYE_HT_CM1[21],
            asKRSummaryLog.EYE_HT_CM1[22], asKRSummaryLog.EYE_HT_CM1[23],
            asKRSummaryLog.EYE_HT_CM1[24], asKRSummaryLog.EYE_HT_CM1[25],
            asKRSummaryLog.EYE_HT_CM1[26], asKRSummaryLog.EYE_HT_CM1[27],
            asKRSummaryLog.EYE_HT_CM1[28], asKRSummaryLog.EYE_HT_CM1[29],
            asKRSummaryLog.EYE_HT_CM1[30], asKRSummaryLog.EYE_HT_CM1[31],
            asKRSummaryLog.EYE_HT_CM1[32], asKRSummaryLog.EYE_HT_CM1[33],
            asKRSummaryLog.EYE_HT_CM1[34], asKRSummaryLog.EYE_HT_CM1[35],
            asKRSummaryLog.EYE_HT_CM1[36], asKRSummaryLog.EYE_HT_CM1[37],
            asKRSummaryLog.EYE_HT_CM1[38], asKRSummaryLog.EYE_HT_CM1[39],
            asKRSummaryLog.EYE_HT_CM1[40], asKRSummaryLog.EYE_HT_CM1[41],
            asKRSummaryLog.EYE_HT_CM1[42], asKRSummaryLog.EYE_HT_CM1[43],
            asKRSummaryLog.EYE_HT_CM1[44], asKRSummaryLog.EYE_HT_CM1[45],
            asKRSummaryLog.EYE_HT_CM1[46], asKRSummaryLog.EYE_HT_CM1[47],
            asKRSummaryLog.EYE_HT_CM1[48], asKRSummaryLog.EYE_HT_CM1[49],
            asKRSummaryLog.EYE_HT_CM1[50], asKRSummaryLog.EYE_HT_CM1[51],
            asKRSummaryLog.EYE_HT_CM1[52], asKRSummaryLog.EYE_HT_CM1[53],
            asKRSummaryLog.EYE_HT_CM1[54], asKRSummaryLog.EYE_HT_CM1[55],
            asKRSummaryLog.EYE_HT_CM1[56], asKRSummaryLog.EYE_HT_CM1[57],
            asKRSummaryLog.EYE_HT_CM1[58], asKRSummaryLog.EYE_HT_CM1[59],
            asKRSummaryLog.EYE_HT_CM1[60], asKRSummaryLog.EYE_HT_CM1[61],
            asKRSummaryLog.EYE_HT_CM1[62], asKRSummaryLog.EYE_HT_CM1[63],
            asKRSummaryLog.EYE_HT_C0[0], asKRSummaryLog.EYE_HT_C0[1], asKRSummaryLog.EYE_HT_C0[2],
            asKRSummaryLog.EYE_HT_C0[3], asKRSummaryLog.EYE_HT_C0[4], asKRSummaryLog.EYE_HT_C0[5],
            asKRSummaryLog.EYE_HT_C0[6], asKRSummaryLog.EYE_HT_C0[7], asKRSummaryLog.EYE_HT_C0[8],
            asKRSummaryLog.EYE_HT_C0[9], asKRSummaryLog.EYE_HT_C0[10], asKRSummaryLog.EYE_HT_C0[11],
            asKRSummaryLog.EYE_HT_C0[12], asKRSummaryLog.EYE_HT_C0[13],
            asKRSummaryLog.EYE_HT_C0[14], asKRSummaryLog.EYE_HT_C0[15],
            asKRSummaryLog.EYE_HT_C0[16], asKRSummaryLog.EYE_HT_C0[17],
            asKRSummaryLog.EYE_HT_C0[18], asKRSummaryLog.EYE_HT_C0[19],
            asKRSummaryLog.EYE_HT_C0[20], asKRSummaryLog.EYE_HT_C0[21],
            asKRSummaryLog.EYE_HT_C0[22], asKRSummaryLog.EYE_HT_C0[23],
            asKRSummaryLog.EYE_HT_C0[24], asKRSummaryLog.EYE_HT_C0[25],
            asKRSummaryLog.EYE_HT_C0[26], asKRSummaryLog.EYE_HT_C0[27],
            asKRSummaryLog.EYE_HT_C0[28], asKRSummaryLog.EYE_HT_C0[29],
            asKRSummaryLog.EYE_HT_C0[30], asKRSummaryLog.EYE_HT_C0[31],
            asKRSummaryLog.EYE_HT_C0[32], asKRSummaryLog.EYE_HT_C0[33],
            asKRSummaryLog.EYE_HT_C0[34], asKRSummaryLog.EYE_HT_C0[35],
            asKRSummaryLog.EYE_HT_C0[36], asKRSummaryLog.EYE_HT_C0[37],
            asKRSummaryLog.EYE_HT_C0[38], asKRSummaryLog.EYE_HT_C0[39],
            asKRSummaryLog.EYE_HT_C0[40], asKRSummaryLog.EYE_HT_C0[41],
            asKRSummaryLog.EYE_HT_C0[42], asKRSummaryLog.EYE_HT_C0[43],
            asKRSummaryLog.EYE_HT_C0[44], asKRSummaryLog.EYE_HT_C0[45],
            asKRSummaryLog.EYE_HT_C0[46], asKRSummaryLog.EYE_HT_C0[47],
            asKRSummaryLog.EYE_HT_C0[48], asKRSummaryLog.EYE_HT_C0[49],
            asKRSummaryLog.EYE_HT_C0[50], asKRSummaryLog.EYE_HT_C0[51],
            asKRSummaryLog.EYE_HT_C0[52], asKRSummaryLog.EYE_HT_C0[53],
            asKRSummaryLog.EYE_HT_C0[54], asKRSummaryLog.EYE_HT_C0[55],
            asKRSummaryLog.EYE_HT_C0[56], asKRSummaryLog.EYE_HT_C0[57],
            asKRSummaryLog.EYE_HT_C0[58], asKRSummaryLog.EYE_HT_C0[59],
            asKRSummaryLog.EYE_HT_C0[60], asKRSummaryLog.EYE_HT_C0[61],
            asKRSummaryLog.EYE_HT_C0[62], asKRSummaryLog.EYE_HT_C0[63],
            asKRSummaryLog.EYE_HT_CP1[0], asKRSummaryLog.EYE_HT_CP1[1],
            asKRSummaryLog.EYE_HT_CP1[2], asKRSummaryLog.EYE_HT_CP1[3],
            asKRSummaryLog.EYE_HT_CP1[4], asKRSummaryLog.EYE_HT_CP1[5],
            asKRSummaryLog.EYE_HT_CP1[6], asKRSummaryLog.EYE_HT_CP1[7],
            asKRSummaryLog.EYE_HT_CP1[8], asKRSummaryLog.EYE_HT_CP1[9],
            asKRSummaryLog.EYE_HT_CP1[10], asKRSummaryLog.EYE_HT_CP1[11],
            asKRSummaryLog.EYE_HT_CP1[12], asKRSummaryLog.EYE_HT_CP1[13],
            asKRSummaryLog.EYE_HT_CP1[14], asKRSummaryLog.EYE_HT_CP1[15],
            asKRSummaryLog.EYE_HT_CP1[16], asKRSummaryLog.EYE_HT_CP1[17],
            asKRSummaryLog.EYE_HT_CP1[18], asKRSummaryLog.EYE_HT_CP1[19],
            asKRSummaryLog.EYE_HT_CP1[20], asKRSummaryLog.EYE_HT_CP1[21],
            asKRSummaryLog.EYE_HT_CP1[22], asKRSummaryLog.EYE_HT_CP1[23],
            asKRSummaryLog.EYE_HT_CP1[24], asKRSummaryLog.EYE_HT_CP1[25],
            asKRSummaryLog.EYE_HT_CP1[26], asKRSummaryLog.EYE_HT_CP1[27],
            asKRSummaryLog.EYE_HT_CP1[28], asKRSummaryLog.EYE_HT_CP1[29],
            asKRSummaryLog.EYE_HT_CP1[30], asKRSummaryLog.EYE_HT_CP1[31],
            asKRSummaryLog.EYE_HT_CP1[32], asKRSummaryLog.EYE_HT_CP1[33],
            asKRSummaryLog.EYE_HT_CP1[34], asKRSummaryLog.EYE_HT_CP1[35],
            asKRSummaryLog.EYE_HT_CP1[36], asKRSummaryLog.EYE_HT_CP1[37],
            asKRSummaryLog.EYE_HT_CP1[38], asKRSummaryLog.EYE_HT_CP1[39],
            asKRSummaryLog.EYE_HT_CP1[40], asKRSummaryLog.EYE_HT_CP1[41],
            asKRSummaryLog.EYE_HT_CP1[42], asKRSummaryLog.EYE_HT_CP1[43],
            asKRSummaryLog.EYE_HT_CP1[44], asKRSummaryLog.EYE_HT_CP1[45],
            asKRSummaryLog.EYE_HT_CP1[46], asKRSummaryLog.EYE_HT_CP1[47],
            asKRSummaryLog.EYE_HT_CP1[48], asKRSummaryLog.EYE_HT_CP1[49],
            asKRSummaryLog.EYE_HT_CP1[50], asKRSummaryLog.EYE_HT_CP1[51],
            asKRSummaryLog.EYE_HT_CP1[52], asKRSummaryLog.EYE_HT_CP1[53],
            asKRSummaryLog.EYE_HT_CP1[54], asKRSummaryLog.EYE_HT_CP1[55],
            asKRSummaryLog.EYE_HT_CP1[56], asKRSummaryLog.EYE_HT_CP1[57],
            asKRSummaryLog.EYE_HT_CP1[58], asKRSummaryLog.EYE_HT_CP1[59],
            asKRSummaryLog.EYE_HT_CP1[60], asKRSummaryLog.EYE_HT_CP1[61],
            asKRSummaryLog.EYE_HT_CP1[62], asKRSummaryLog.EYE_HT_CP1[63], asKRSummaryLog.LD_CM1,
            asKRSummaryLog.LD_C0, asKRSummaryLog.LD_CP, asKRSummaryLog.LD_VGA,
            asKRSummaryLog.LD_EDC, asKRSummaryLog.LD_EQR, asKRSummaryLog.FEC_CORRECTION,
            asKRSummaryLog.FEC_UNCORRECTION, asKRSummaryLog.CURR_EYE_HT,
            asKRSummaryLog.TRAINING_TIME_MS,
            ((asKRSummaryLog.wAneg_status & 0x80) == 0x80)
                ? "Yes"
                : (((asKRSummaryLog.wAneg_status & 0x80) != 0x80) ? "NO" : "NA"),
    ...
    for (int iport = 0; iport < meba_phy_kr_inst->phy_device_cnt - 1; iport++) {
        ...
        if (base_data->krlog_en_ports != u8KRLogEnabledPorts) {
            ...
            for (int byIndex = 0; byIndex < MAX_PORTS; byIndex++) {
                if (u8KRLogEnabledPorts & (1 << byIndex)) {
                    lineMemory[base_data->channel_id - byIndex] =
                        KR_LOG_BASE_SUMMARY_PORT0 + (u16DataLength * byPartitionCount);
                    byPartitionCount += 1;
                }
            }
            ...
            for (int byIndex = 0; byIndex < MAX_PORTS; byIndex++) {
                if (u8KRLogEnabledPorts & (1 << (byIndex + 4))) {
                    hostMemory[base_data->channel_id - byIndex] =
                        KR_LOG_BASE_SUMMARY_PORT0 + (u16DataLength * byPartitionCount);
                    byPartitionCount += 1;
                }
            }
        ...
        if (mreq->line_port == 1) {
            if (u8KRLogEnabledPorts & (1 << data->channel_id)) {
                if (mreq->clr) {
                    if ((rc = lan80xx_KRLog_Reset(meba_phy_kr_inst->phy_devices[iport],
                    ...
                    processKrLogging(meba_phy_kr_inst->phy_devices[iport], mreq, iport, 0,
                                     lineMemory[iport], u16DataLength);
                }
            } else {
        ...
        if (mreq->host_port == 1) {
            if (u8KRLogEnabledPorts & (1 << (data->channel_id + 4))) {
                if (mreq->clr) {
                    ...
                    processKrLogging(meba_phy_kr_inst->phy_devices[iport], mreq, iport, 1,
                                     hostMemory[iport], u16DataLength);
                }
            } else {
...
static cli_cmd_t cli_cmd_table[] = {
    {"PHY KR aneg [<port_list>] [adv-1g] [adv-10g] [adv-25g] [adv-25g-krs] [rfec-10g] [rfec-25g] [rsfec] [np] [np-rfec] [np-rsfec] [fw-res] [train] [disable]",
     "Set or show kr",                                                                                                                                                                          cli_cmd_phy_kr            },
    {"PHY SERDES get <speed_idx>",                                                                                                                              "Get phy serdes configuration", cli_cmd_phy_serdes_get    },
    {"PHY SERDES set <speed_idx>",                                                                                                                              "Set phy serdes configuration", cli_cmd_phy_serdes_set    },
    {"PHY KR_LOG enable <port_list> [host] [line]",                                                                                                             "Enable KR Log",                cli_cmd_phy_kr_log_enable },
    {"PHY KR_LOG disable <port_list> [host] [line]",                                                                                                            "Disable KR Log",               cli_cmd_phy_kr_log_disable},
    {"PHY KR_LOG status <port_list> [host] [line] [aneg|eq|ber|irq|all|clr]",                                                                                   "Show KR Log",
     cli_cmd_phy_kr_logging                                                                                                                                                                                               },
};

static cli_parm_t cli_parm_table[] = {
    {"adv-1g", "adv-1g: advertise 1g", CLI_PARM_FLAG_NO_TXT | CLI_PARM_FLAG_SET,
     cli_kr_parm_keyword, cli_cmd_phy_kr},
    {"adv-10g", "adv-10g: advertise 10g", CLI_PARM_FLAG_NO_TXT | CLI_PARM_FLAG_SET,
     cli_kr_parm_keyword, cli_cmd_phy_kr},
    {"adv-25g", "adv-25g: advertise 25g kr", CLI_PARM_FLAG_NO_TXT | CLI_PARM_FLAG_SET,
     cli_kr_parm_keyword, cli_cmd_phy_kr},
    {"adv-25g-krs", "adv-25g-krs: advertise 25g krs", CLI_PARM_FLAG_NO_TXT | CLI_PARM_FLAG_SET,
     cli_kr_parm_keyword, cli_cmd_phy_kr},

    {"rfec-10g", "rfec: advertise 10g r-fec capability", CLI_PARM_FLAG_NO_TXT | CLI_PARM_FLAG_SET,
     cli_kr_parm_keyword, cli_cmd_phy_kr},
    {"rfec-25g", "rfec: advertise 25g r-fec capability", CLI_PARM_FLAG_NO_TXT | CLI_PARM_FLAG_SET,
     cli_kr_parm_keyword, cli_cmd_phy_kr},

    {"rsfec", "rs-fec: advertise 25g rs-fec capability", CLI_PARM_FLAG_NO_TXT | CLI_PARM_FLAG_SET,
     cli_kr_parm_keyword, cli_cmd_phy_kr},
    {"np", "np: use next page for advertise", CLI_PARM_FLAG_NO_TXT | CLI_PARM_FLAG_SET,
     cli_kr_parm_keyword, cli_cmd_phy_kr},
    {"np-rfec", "np r-fec: advertise next page r-fec capability",
     CLI_PARM_FLAG_NO_TXT | CLI_PARM_FLAG_SET, cli_kr_parm_keyword, cli_cmd_phy_kr},
    {"np-rsfec", "np rs-fec: advertise next page rs-fec capability",
     CLI_PARM_FLAG_NO_TXT | CLI_PARM_FLAG_SET, cli_kr_parm_keyword, cli_cmd_phy_kr},
    {"fw-res", "fw-res: enable firmware resolve", CLI_PARM_FLAG_NO_TXT | CLI_PARM_FLAG_SET,
     cli_kr_parm_keyword, cli_cmd_phy_kr},

    {"train", "train: enable training", CLI_PARM_FLAG_NO_TXT | CLI_PARM_FLAG_SET,
     cli_kr_parm_keyword, cli_cmd_phy_kr},
    {"disable", "disable: disable kr", CLI_PARM_FLAG_NO_TXT | CLI_PARM_FLAG_SET,
     cli_kr_parm_keyword, cli_cmd_phy_kr},
    {
     "<speed_idx>", "speed_idx: Speed index 0 - 1G, 1 - 10G, 2 - 25G",
     CLI_PARM_FLAG_NO_TXT | CLI_PARM_FLAG_SET,
     cli_parm_sd_speed_idx, },
    {
     "aneg|eq|ber|irq|all|clr", "aneg   : ANEG Log"
