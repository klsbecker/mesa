...
static mscc_appl_trace_module_t trace_module = {.name = "phy_diagnostics"};
...
static cli_cmd_t cli_cmd_table[] = {
    {"PHY Cable_diagnostics <port_no>", "start cable diagnostics for particular port number of phy",
...
static void cli_cmd_cable_diagnostics(cli_req_t *req)
{
    ...
    for (int i = 0; i < 4; i++) {
        cli_printf("%-6d %-6s %-10s %-6d\n", result.link, PairNameStrings[i],
                   CableDiagStatusStrings[result.status[i]], result.length[i]);
    }
    return;
}
