...
static mscc_appl_trace_module_t trace_module = {.name = "gpio_lp_app"};
...
static int phy_gpio_no_for_alternate_function(phy_family_t  family,
                                              int           alt_fun,
...
static mepa_rc lan80xx_gpio_fn_channel_map(mepa_device_t *dev, const uint8_t gpio_num)
{
...
static void cli_cmd_gpio_conf(cli_req_t *req)
{
    ...
    if ((phy_family.family == PHY_FAMILY_VIPER) || (phy_family.family == PHY_FAMILY_TESLA) ||
        (phy_family.family == PHY_FAMILY_LAN8814)) {
        ...
        if (gpio_conf.mode == MEPA_GPIO_MODE_OUT || gpio_conf.mode == MEPA_GPIO_MODE_IN) {
            cli_printf("\n\t Enter GPIO Number to be configured as %s mode: ",
                       gpio_txt[gpio_mode - 1]);
            scanf("%hhu", &gpio_num);
        ...
        if (gpio_mode == 3) {
            ...
            gpio_conf.gpio_no =
                phy_gpio_no_for_alternate_function(phy_family.family, alt_fun, &gpio_table);
            cli_printf("\n\t Selected %s Alternate Mode for GPIO %d PIN\n", gpio_table.desc,
                       gpio_conf.gpio_no);
        }
        /* In Case of LAN8814 PHY, Port LED alternate function the GPIO Number is selected inside
         * the API based on the Channel id of the PHY */
        ...
        if ((rc = mepa_gpio_mode_set(meba_gpio_lp_instance->phy_devices[req->port_no],
                                     &gpio_conf)) != MEPA_RC_OK) {
        ...
        int gpio_output_mode = 0, gpio_out_signal = 0, gpio_alt = 0, gpio_aggr = 0, p_gpio = 0,
            gpio_conf = 0;
        uint8_t      channel_id;
        char         input[3];
        ...
        if (gpio_conf == 1) {
            ...
            case 2:
                ...
                case 1:  gpio_mode.in_sig = VTSS_10G_GPIO_INTR_SGNL_HOST_LINK; break;
                case 2:  gpio_mode.in_sig = VTSS_10G_GPIO_INTR_SGNL_LINE_LINK; break;
                case 3:  gpio_mode.in_sig = VTSS_10G_GPIO_INTR_SGNL_LINE_KR_8b10b_2GPIO; break;
                case 4:  gpio_mode.in_sig = VTSS_10G_GPIO_INTR_SGNL_LINE_KR_10b_2GPIO; break;
                case 5:  gpio_mode.in_sig = VTSS_10G_GPIO_INTR_SGNL_ROSI_PULSE; break;
                case 6:  gpio_mode.in_sig = VTSS_10G_GPIO_INTR_SGNL_ROSI_SDATA; break;
                case 7:  gpio_mode.in_sig = VTSS_10G_GPIO_INTR_SGNL_ROSI_SCLK; break;
                case 8:  gpio_mode.in_sig = VTSS_10G_GPIO_INTR_SGNL_TOSI_PULSE; break;
                case 9:  gpio_mode.in_sig = VTSS_10G_GPIO_INTR_SGNL_TOSI_SCLK; break;
                case 10: gpio_mode.in_sig = VTSS_10G_GPIO_INTR_SGNL_LINE_PCS1G_LINK; break;
                default: T_E("\n Invalid Input \n"); break;
                }
                break;
            default: T_E("\n Invalid Input \n"); return;
            ...
            if (gpio_alt == 10) {
                ...
                case 9:
                    gpio_mode.aggr_intrpt = 1 << ((channel_id * 2) + 1);
                    event_type = MALIBU10G_EXTENDED_EVENT;
                    ext_ev = VTSS_PHY_10G_LINE_MAC_LOCAL_FAULT_EV |
                             VTSS_PHY_10G_LINE_MAC_REMOTE_FAULT_EV;
                    break;
                case 10:
                    gpio_mode.aggr_intrpt = 1 << ((channel_id * 2) + 1);
                    event_type = MALIBU10G_EXTENDED_EVENT;
                    ext_ev = VTSS_PHY_10G_HOST_MAC_LOCAL_FAULT_EV |
                ...
                case 16:
                    ...
                    event_type = MALIBU10G_EXTENDED_EVENT;
                    ext_ev =
                        VTSS_PHY_10G_RX_LOS_EV | VTSS_PHY_10G_RX_LOL_EV | VTSS_PHY_10G_TX_LOL_EV;
                    break;
                case 17:
                    gpio_mode.aggr_intrpt = 1 << (channel_id + 8);
                    gpio_mode.mode = VTSS_10G_PHY_GPIO_1588_INT;
                    event_type = MALIBU10G_PTP_EVENT;
                    ts_ev = VTSS_PHY_TS_EGR_TIMESTAMP_CAPTURED | VTSS_PHY_TS_EGR_ENGINE_ERR |
                            VTSS_PHY_TS_EGR_FIFO_OVERFLOW | VTSS_PHY_TS_INGR_ENGINE_ERR |
                            VTSS_PHY_TS_INGR_RW_FCS_ERR | VTSS_PHY_TS_EGR_RW_FCS_ERR;
                    break;
                case 18: gpio_mode.mode = VTSS_10G_PHY_GPIO_PLL_INT_0; break;
                case 19: gpio_mode.mode = VTSS_10G_PHY_GPIO_PLL_INT_1; break;
                case 20: gpio_mode.mode = VTSS_10G_PHY_GPIO_CRSS_INT; break;
                default: T_E("\n Invalid Input \n"); return;
                }
        ...
        if (event_type == MALIBU10G_EVENT) {
            if (vtss_phy_10g_event_enable_set(NULL, req->port_no, ev, event_ena_dis) !=
                VTSS_RC_OK) {
                T_E("\n vtss_phy_10g_event_enable_set, port %d, gpio %d\n", req->port_no, gpio_no);
                return;
            }
        } else if (event_type == MALIBU10G_EXTENDED_EVENT) {
            if (vtss_phy_10g_extended_event_enable_set(NULL, req->port_no, ext_ev, event_ena_dis) !=
                VTSS_RC_OK) {
            ...
            if (vtss_phy_10g_extended2_event_enable_set(NULL, req->port_no, ex2_ev,
                                                        event_ena_dis) != VTSS_RC_OK) {
                T_E("\n vtss_phy_10g_extended2_event_enable_set, port %d, gpio %d\n", req->port_no,
                    gpio_no);
                return;
            }
        } else if (event_type == MALIBU10G_MACSEC_EVENT) {
            if ((rc = mepa_macsec_event_enable_set(meba_gpio_lp_instance->phy_devices[req->port_no],
                                                   req->port_no, event, event_ena_dis)) !=
                MEPA_RC_OK) {
                T_E("\n Error in Configuring MACsec Event on Port : %d \n", req->port_no);
                return;
            }
        } else if (event_type == MALIBU10G_PTP_EVENT) {
            if (vtss_phy_ts_event_enable_set(NULL, req->port_no, event_ena_dis, ts_ev) !=
                VTSS_RC_OK) {
        ...
        /* Check for Channel Number of the PHY and GPIO Pin selected, this is done as per the
         * LAN80XX EVB Board design */
        if (lan80xx_gpio_fn_channel_map(meba_gpio_lp_instance->phy_devices[req->port_no],
                                        gpio_num) != MEPA_RC_OK) {
        ...
        if (gpio_conf.mode == MEPA_GPIO_MODE_IN) {
            cli_printf("\n");
            cli_printf(
                "\t Enter the GPIO interrupt [0 -  GPIO_INTR_NONE or 1 - GPIO_INTR0 or 2 - GPIO_INTR1 : ");
         ...
         * gpio_conf.pp_enable = Push-Pull or Open Drain, valid only in case of GPIO Pin in Output
         * Mode gpio_conf.gpio_intrpt = Used to route GPIO Input state change Interrupt
         */
        if ((rc = mepa_gpio_mode_set(meba_gpio_lp_instance->phy_devices[req->port_no],
                                     &gpio_conf)) != MEPA_RC_OK) {
...
static void event_poll(meba_inst_t inst)
{
    ...
    for (port_no = 0; port_no < MAX_PORTS; port_no++) {
        ...
        evt_get = 0;
        if ((rc = lan80xx_event_poll(meba_gpio_lp_instance->phy_devices[port_no], port_no,
                                     &evt_get)) != MEPA_RC_OK) {
        ...
        ext_evt_get = 0;
        if ((rc = lan80xx_ext_event_poll(meba_gpio_lp_instance->phy_devices[port_no], port_no,
                                         &ext_evt_get)) != MEPA_RC_OK) {
...
static void cli_cmd_gpio_out_set(cli_req_t *req)
{
    ...
    if ((rc = mepa_gpio_out_set(meba_gpio_lp_instance->phy_devices[req->port_no], mreq->gpio_number,
                                req->enable)) != MEPA_RC_OK) {
...
static void cli_cmd_gpio_in_get(cli_req_t *req)
{
    ...
    if ((rc = mepa_gpio_in_get(meba_gpio_lp_instance->phy_devices[req->port_no], mreq->gpio_number,
                               &value)) != MEPA_RC_OK) {
...
static void cli_cmd_event_conf_set(cli_req_t *req)
{
    ...
    if ((rc = lan80xx_event_conf_get(meba_gpio_lp_instance->phy_devices[req->port_no], req->port_no,
                                     &evt_conf.evt_set)) != MEPA_RC_OK) {
        cli_printf(" Error in Getting Event configuration : %d\n", req->port_no);
        return;
    }
    if ((rc = lan80xx_ext_event_conf_get(meba_gpio_lp_instance->phy_devices[req->port_no],
                                         req->port_no, &ext_evt_conf.ext_evt_set)) != MEPA_RC_OK) {
    ...
    for (evt = 0; evt < mreq->event_cnt; evt++) {
        if (mreq->event_list[evt] <= LAN80XX_MAX_EVENTS) {
            evt_conf.evt_set = req->enable
                                   ? (evt_conf.evt_set | ((uint64_t)1 << mreq->event_list[evt]))
                                   : (evt_conf.evt_set & ~((uint64_t)1 << mreq->event_list[evt]));
        } else if ((mreq->event_list[evt] <= LAN80XX_TOTAL_MAX_EVENTS) &&
                   (mreq->event_list[evt] >= (LAN80XX_TOTAL_MAX_EVENTS - LAN80XX_MAX_EVENTS))) {
            ext_evt_conf.ext_evt_set =
                req->enable ? (ext_evt_conf.ext_evt_set |
                               ((uint64_t)1 << (mreq->event_list[evt] -
                                                (LAN80XX_TOTAL_MAX_EVENTS - LAN80XX_MAX_EVENTS))))
                            : (ext_evt_conf.ext_evt_set &
                               ~((uint64_t)1 << (mreq->event_list[evt] -
                                                 (LAN80XX_TOTAL_MAX_EVENTS - LAN80XX_MAX_EVENTS))));
        } else {
    ...
    if ((rc = lan80xx_event_conf_set(meba_gpio_lp_instance->phy_devices[req->port_no], req->port_no,
                                     evt_conf)) != MEPA_RC_OK) {
        cli_printf(" Error in Event configuration : %d\n", req->port_no);
        return;
    }
    if ((rc = lan80xx_ext_event_conf_set(meba_gpio_lp_instance->phy_devices[req->port_no],
                                         req->port_no, ext_evt_conf)) != MEPA_RC_OK) {
...
static cli_cmd_t cli_cmd_table[] = {
    {
     "PHY GPIO_CONF <port_no>",              "GPIO Configuration",
     cli_cmd_gpio_conf, },

    {
     "PHY GPIO_OUT_SET <port_no> <gpio_num> [enable|disable]",                                     "GPIO Output Pin Set",
     cli_cmd_gpio_out_set, },

    {
     "PHY GPIO_IN_GET <port_no> <gpio_num>","GPIO Value Get",
     cli_cmd_gpio_in_get, },

    {
     "PHY GPIO_TOGGLE <port_no> <gpio_num>",                                       "GPIO toggle",
     cli_cmd_gpio_toggle, },

    {
     "PHY Event_set <port_no> <event_list> [enable|disable] [intr_a|intr_b]",                   "Event set configuration",
     cli_cmd_event_conf_set, },

    {
     "PHY Event_poll <port_no> [enable|disable]", "Poll all events",
     cli_cmd_event_poll, },
};
...
static cli_parm_t cli_parm_table[] = {
    {
     ...
     CLI_PARM_FLAG_NONE, cli_param_parse_gpio_no,
     },

    {
     "<event_list>", "event_list   : Set the events and extended events list\n"
...
void mepa_demo_appl_gpio_lp_demo(mscc_appl_init_t *init)
{
    ...
    case MSCC_INIT_CMD_POLL: event_poll(init->board_inst); break;
    default:                 break;
    }
}
