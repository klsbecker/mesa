...
== Directory
* `./mesa/demo/mepa_apps/phy_macsec_demo.h` - Header File for MACsec Demo Application
* `./mesa/demo/mepa_apps/phy_macsec_demo.c` - Source File for MACsec Demo Application
* `./mesa/demo/mepa_scripts/macsec_key.json` - JSON file for MACsec Key, Application will take Key from this File

== Prerequisites
MACsec Demo Application to run on EDSx Platform User needs to flash `ext4` image on 
...
== Supported PHYs
    ...
    * The MACsec Demo application is supported for all MEPA supported PHYs

== Procedure to Start/Exit MACsec Demo Application

To Run MACsec Demo Application the 'mesa-demo' application needs to be running.
Reffer Section link:#mesa/demo/docs/getting-started[Getting-Started] to know about the procedure 
to run 'mesa-demo' Application.

Once 'mesa-demo' application is started the MACsec Demo Application can be started with the command
'mesa-cmd PHY Macsec'.
...
bypass the MACsec Block.
  ...
  * mesa-cmd macsec enable <port_list> [bypass_none|bypass_disable] - `Enable MACsec Block`
  * mesa-cmd macsec disable <port_list> [bypass_none|bypass_disable] - `Disable/Bypass MACsec Block`
...
identified by unique `port_id` value.
  ...
  * mesa-cmd secy_create_upd  create <port_no> port_id <port_id> mac-addr <00-00-00-00-00-00> - `Create SecY`
  * mesa-cmd secy_create_upd  update <port_no> port_id <port_id> mac-addr <00-00-00-00-00-00> - `Update SecY`
...
----
# mesa-cmd secy_create_upd create 13 port-id 4 mac-addr 00-00-00-00-00-01
...
The Pattern matching parameters can be provide with help of following command.
    ...
    * mesa-cmd match_conf <port_no> ethtype <ethertype> src_mac <XX-XX-XX-XX-XX-XX> dst_mac <XX-XX-XX-XX-XX-XX> vlanid <vlanid> vid-inner <inner_vlanid>
...
to be called as mandatory to configure the Egress/ingress and pattern matching configuration.
    ...
    * mesa-cmd pattern_conf <port_no> port_id <port_id> [egress | ingress] [cont_port| uncont_port]

Following is the Example to configure the Pattern Matching for MACsec Block.

----
# mesa-cmd match_conf 13 ethtype 2048 src_mac 00-01-02-03-04-04 dst_mac 00-02-03-04-05-06 vlanid 0 vid-inner 0
 ...... Match Parameters configured......
#
#
#
# mesa-cmd pattern_conf 13 port-id 4 egress cont_port
...
Transmit Secure channel.
    ...
    * mesa-cmd tx_sc_create <port_no> port_id <port_id> - `Create Tx Secure channel`
    * mesa-cmd tx_sc_del <port_no> port_id <port_id> - `Delete Tx Secure Channel`
...
Secure Association in each secure channel is identified by unique Association Number (AN) which can be from 0 to 3.
    ...
    * mesa-cmd tx_sa_create <port_no> port_id <port_id> an <an> next_pn <pn> conf [true | false] - `Create Tx SA`
...
open a file `macsec_key.json` available in path `./mesa/demo/mepa_scripts/` and read the value of SAK and configure.
...
secy create command and 16-bit channel identifier(`sc_id`).
    ...
    * mesa-cmd rx_sc_create <port_no> port_id <port_id> sc_id <id> 

To Delete the Receive Secure Channel use the below Command 

    * mesa-cmd rx_sc_del <port_no> port_id <port_id> sc_id <id>
...
Similar to Tx SA Create Receive Secure Association Create will open a JSON file provided as input to get the SAK values.
    ...
    * mesa-cmd rx_sa_create <port_no> port_id <port_id> sc_id <id> an <an> lowest_pn <pn> - `Create Rx SA`
    * mesa-cmd rx_sa_del <port_no> port_id <port_id> sc_id <id> an <an> - `Delete Rx SA`

The argument `lowest_pn` is the value of Packet Number the MACsec Block is Expecting to Decrypt next.

----
# mesa-cmd rx_sc_create 19 port-id 4 sc-id 1
...
#
#
# mesa-cmd rx_sa_create 19 port-id 4 sc-id 1 an 0 lowest-pn 1
...
packets that each SecY of MACsec Block Processed.
    ...
    * mesa-cmd statistics secy <port_no> port_id <port_id> get                     - `SecY Statistics`
    * mesa-cmd statistics tx_sc <port_no> port_id <port_id> get                    - `Tx SC Statistics`
    * mesa-cmd statistics tx_sa <port_no> port_id <port_id> an <an> get            - `Tx SA Statistics`
    * mesa-cmd statistics rx_sc <port_no> port_id <port_id> sc_id <id> get         - `Rx SC Statistics`
    * mesa-cmd statistics rx_sa <port_no> port_id <port_id> sc_id <id> an <an> get - `Rx SA Statistics`
...
#// SecY Statistics Get
#
# mesa-cmd statistics secy 13 port-id 4 get
...
#
#
# mesa-cmd statistics tx_sc 13 port-id 4 get
...
#
#
# mesa-cmd statistics tx_sa 13 port-id 4 an 1 get
...
#
# mesa-cmd statistics rx_sc 13 port-id 4 sc-id 2 get
...
The MACsec Statistics Counters can be Cleared with help of following Commands
    ...
    * mesa-cmd statistics secy <port_no> port_id <port_id> clear                     - `Clear SecY Statistics`
    * mesa-cmd statistics tx_sc <port_no> port_id <port_id> clear                    - `Clear Tx SC Statistics`
    * mesa-cmd statistics tx_sa <port_no> port_id <port_id> an <an> clear            - `Clear Tx SA Statistics`
    * mesa-cmd statistics rx_sc <port_no> port_id <port_id> sc_id <id> clear         - `Clear Rx SC Statistics`
    * mesa-cmd statistics rx_sa <port_no> port_id <port_id> sc_id <id> an <an> clear - `Clear Rx SA Statistics`
...
it also provides the packet error counters.
    ...
    * mesa-cmd statistics mac hmac <port_no> get    - `Host MAC Statistics Get`
    * mesa-cmd statistics mac lmac <port_no> get    - `Line MAC Statistics Get`
    * mesa-cmd statistics mac hmac <port_no> clear  - `Host MAC Statistics Clear`
    * mesa-cmd statistics mac lmac <port_no> clear  - `Line MAC Statistics Clear`

The following example will provide getting Host MAC and Line MAC Statistics
----
#
# mesa-cmd statistics mac hmac 13 get
...
#
#
# mesa-cmd statistics mac lmac 13 get
...
The following command will configure the next Expected Packet Number by the MACsec Block in Transform Record Register.
    ...
    * mesa-cmd rx_sa_pn_upd <port_no> port-id <port_id> sc-id <id> an <an_no> lowest-pn <pn>
...
which has a argument '[egress|ingress]' to select the direction of Packet capture.
    ...
    * mesa-cmd frame capture <port_list> [egress|ingress]
...
The following command is used to enable the MACsec Event.
    ...
    * mesa-cmd macsec event set <port_list> [rollover|seq_threshold] [evt_enable|evt_disable]
...
# mesa-cmd macsec event set 13,14 rollover evt_enable
#
#
# mesa-cmd macsec event get
...
# mesa-cmd macsec event set 13,14 seq_threshold evt_enable
#
#
#
# mesa-cmd macsec event get
...
input provided by the user.
    ...
    * mesa-cmd conf_get [secy|tx_sc|tx_sa|rx_sc|rx_sa] <port_no> port-id <port_id> sc-id <id> an <an_no>

Following are the commands to get the different configurations.

    * mesa-cmd conf_get secy <port_no> port-id <port_id> sc-id <id> an <an_no> - `SecY Conf Get`
    * mesa-cmd conf_get tx_sc <port_no> port-id <port_id> sc-id <id> an <an_no> - `Tx SC Conf Get`
    * mesa-cmd conf_get tx_sa <port_no> port-id <port_id> sc-id <id> an <an_no> - `Tx SA Conf Get`
    * mesa-cmd conf_get rx_sc <port_no> port-id <port_id> sc-id <id> an <an_no> - `Rx SC Conf Get`
    * mesa-cmd conf_get rx_sa <port_no> port-id <port_id> sc-id <id> an <an_no> - `Rx SA Conf get`
...
#
#
# mesa-cmd conf_get secy 13 port-id 4 sc-id 0 an 0
...
#
#
# mesa-cmd conf_get tx_sc 13 port-id 4 sc-id 0 an 0
...
#
#
# mesa-cmd conf_get tx_sa 13 port-id 4 sc-id 0 an 1
...
#
#
# mesa-cmd conf_get rx_sc 13 port-id 4 sc-id 5 an 0
...
#
#
# mesa-cmd conf_get rx_sa 13 port-id 4 sc-id 5 an 2
...
without encrypting and without checking the integrity, it will pass the packet through uncontrolled port.
    ...
    * mesa-cmd ctrl_frame set <port_no> ethtype <ether_type> dst_mac <mac_address>

MACsec Engine Supports either Matching of DMAC or Ethertype or Both which will be one of the input of this Command.

----
# mesa-cmd ctrl_frame set 13 ethtype 2048 dst_mac 00-01-00-00-00-00
...
It will list all the control traffic configuration on the port (List all rule id configuration).
    ...
    * mesa-cmd ctrl_frame get <port_no>
...
#
#
# mesa-cmd ctrl_frame set 13 ethtype 2054 dst_mac 00-01-00-00-00-00
...
#
#
# mesa-cmd ctrl_frame set 13 ethtype 2054 dst_mac 00-01-00-00-00-03
...
The Command `ctrl_frame_match_del` is used to delete the specified pattern matching rule on the MACsec Enabled port.
    ...
    * mesa-cmd ctrl_frame del <port_no> <rule_list>
...
#
#
# mesa-cmd ctrl_frame del 13 0,3
#
#
#
# mesa-cmd ctrl_frame get 13
...
#
#
# mesa-cmd tx_sc_create 13 port-id 4
...
to 4 VLAN Tags.
    ...
    * mesa-cmd vlan_bypass <port_no>  port-id <port_id> [zero|one|two|three|four]
...
#
#
# mesa-cmd macsec enable 13 bypass_none
...
#
#
# mesa-cmd secy_create_upd create 13 port-id 4 mac-addr 00-01-02-03-04-05
...
#
#
# mesa-cmd match_conf 13 ethtype 2048 src_mac 00-00-00-00-00-00 dst_mac 00-00-00
...
#
#
# mesa-cmd match_conf 13 ethtype 35045 src_mac 00-00-00-00-00-00 dst_mac 00-00-00
-00-00-00 vlanid 0 vid-inner 0
...
#
#
# mesa-cmd rx_sc_create 13 port-id 4 sc-id 5

 ...... Receive Secure Channel Created on port 12 for SecY with Port id :4......
#
#
#
# mesa-cmd rx_sa_create 13 port-id 4 sc-id 5 an 2 lowest-pn 1
