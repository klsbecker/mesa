...
== Directory
* `./mesa/demo/mepa_apps/phy_loopback.c` - Source Code for Loopback Demo Sample Application.
* `./mesa/demo/mepa_scripts/loopback_config.json` - JSON file for Loopback config, Application will take configuration from this File
...
To Run Loopback Demo Application the 'mesa-demo' application needs to be running.
...
Once 'mesa-demo' application is started then we can run the Loopback Demo Application
with help of following Command.

    mesa-cmd PHY Loopback <port_no> -f <file_name>
...
----
# mesa-cmd port stati 13 clear
#
# mesa-cmd packet tx 13 100 100
Sending 100 frames of 100 bytes on port 13
#
#
#
# mesa-cmd port stati 13
Port 21 Statistics:
