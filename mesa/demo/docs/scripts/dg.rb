...
puts $top
$mepa_top = "#{$top}/mesa/demo"
$doc_top = "#{$top}/mesa/demo/docs"
...
$conf = YAML::load(File.open('mesa/demo/docs/sitemap.yaml'))
...
def process_adoc dir, base, ext, content
    ...
    xml.css("code").each do |e|
        ...
        when /^(mepa_\w+)::(\w+)$/, /^(mepa_\w+)::(\w+)\([^\)]*\)$/, /^(vtss_\w+)::(\w+)$/, /^(lan80xx_\w+)::(\w+)$/, /^(phy_25g_\w+)::(\w+)$/, /^(phy25g_\w+)::(\w+)$/,
...
IO.binwrite("#{$mepa_top}/mepa-app-doc.html", html)
