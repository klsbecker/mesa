...
The API `mepa_capability()` allows to interogate the PHY capabilities. These
will describe for example which features like speed, timestamping, cable
diagnostics, etc are supported by the PHY.

NOTE: For now `mepa_capability()` returns the same capabilities as the function
`mepa_phy_info_get()`. In the future new capabilities will be added to
`mepa_capability()` and the capabilities from `mepa_phy_info_get` will be
removed.
