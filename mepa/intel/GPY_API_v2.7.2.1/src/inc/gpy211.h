...
/** \brief Cable diagnostic report */
struct gpy211_sck_cdiag_report {
...
/**@{*/
/**
	\brief API to configure GPIO pad voltage.

	\param phy Pointer to GPHY data (\ref gpy211_device).

	\param u8 supplylevel. The user application should give
	proper supply leve1.
		0	HIGH min 2.5v
		1	LOW max 1.8v
	\return
...
int gpy2xx_gpio_pad_vol_cfg(struct gpy211_device *phy, u8 supplylevel);

/**
	\brief API to get GPIO pad voltage.
	...
	- <0: error code
*/
int gpy2xx_gpio_pad_vol_cfg_get(struct gpy211_device *phy);
...
/**
	\brief Gets the GPY2xx's super isolate mode Enable/Disable.

	\param phy Pointer to GPHY data (\ref gpy211_device).
	\param enable - u8 variable to enable/disable super isolate mode 0 - disable , 1 - enable (\ref enable).
	\return
	- =: successful
	- <0: error code
*/
...
/**
	\brief Gets the GPY2xx's Super isolate mode Enable/Disable state.

	\param phy Pointer to GPHY data (\ref gpy211_device).
	\return
	- =0/1: 1 - super isolate mode enabled , 0 - super isolate mode disabled 
	- <0: error code
*/
int gpy2xx_super_isolate_mode_get(struct gpy211_device *phy);

/**
	\brief Gets the GPY2xx's DVS Enable/Disable state.

	\param phy Pointer to GPHY data (\ref gpy211_device).
	\param disable - u8 variable to disable/enable dvs 1 - disable , 0 - enable (\ref disable).
	\return
	- =: successful
	- <0: error code
*/
...
/**
	...
	\param phy Pointer to GPHY data (\ref gpy211_device).
	\return
	- =0/1: 1 - dvs disabled , 0 - dvs enabled 
	- <0: error code
*/
...
/**
	...
	\details This function notifies the PHY to start the firmware download.
	 The API will initiate the sram download by sending a SRAM download command
	 to the PHY , the PHY will recognize this command and will prepare the
	 environement for SRAM download and return an acknowledgement that the 
	 phy is ready for FW download.
	...
	\return
	- =0xA5A5 : sram_download_init_success code
	- <0      : error code
*/
int gpy2xx_fw_sram_dl_init(struct gpy211_device *phy, u32 timeout);

/**
	\brief API to start to write data to the PHY internal sram. 
	\details This function notifies the PHY to start the firmware download.
	 The user should allocate the buffer as per the data size (fw size) and 
	 call this function with the buffer pointer and the buffer size
	 the API will write the data to the internal phy sram. 
	\return
	- =0: successful
	- <0: error code
*/
...
/**
	\brief API to stop the FW downloading procedure.
	\details This function notifies the PHY to stop the firmware.
	 This API will inform the PHY that the host has finished writing data
	 to the PHY sram
	\return
	- =0: successful
	- <0: error code
