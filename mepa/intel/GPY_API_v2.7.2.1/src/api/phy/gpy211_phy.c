...
int __gpy211_config_init(struct gpy211_device *phy)
{
	...
	/*0x00984FEE - Legacy products*/
	/*0x000C32AB - Swvl2*/
	if ((data->id.OUI != 0x00984FEE) && (data->id.OUI != 0x000C32AB)) {
		LOG_CRIT("ERROR: OUI mismatch: 0x%x != 0x00984FEE\n", data->id.OUI);
		LOG_CRIT("ERROR: OUI mismatch: 0x%x != 0x000C32AB\n", data->id.OUI);
		return -ENXIO;
	}
	...
	/* Engineering-sample silicon reports family=0xF; chip is a real GPY24x
	 * (e.g. GPY241B0 with model_no=0x24). Treat as P34X so all the family
	 * gating in this SDK works. v2.7.1.1 had an explicit branch for this. */
	if (data->id.family == 0xF)
		data->id.family = ID_P34X;
...
int gpy2xx_sck_cdiag_start(struct gpy211_device *phy)
{
	int ret;
	u16 cmd = 0;
	u16 addr_low = 0x8000; /* Run immediate: 1 = run now (self clearing), 0 = no action */
	LOG_INFO("Entering func name :- %s\n", __func__);
	...
	/* Enter Cable Diagnosis command, delay 10ms to make sure finish */
	FIELD_REPLACE(cmd, MBOX_CMD_CDIAG_MODE, VSPEC1_MBOX_CMD_CMD);
	LOG_INFO("CDIAG: phy=%x, MDIO_MMD_VEND1=%x, VSPEC1_MBOX_CMD=%x, cmd=%x\n", (unsigned int)phy->phy_addr, (unsigned int)MDIO_MMD_VEND1, (unsigned int)VSPEC1_MBOX_CMD, (unsigned int)cmd);
	ret = PHY_WRITE_MMD(phy, MDIO_MMD_VEND1, VSPEC1_MBOX_CMD, cmd);

	udelay(10);

	/* start Cable Diagnosis test  */
	FIELD_REPLACE(cmd, UTAPI_ID_CDIAG_START, VSPEC1_MBOX_CMD_ADDRHI);
	FIELD_REPLACE(cmd, MBOX_CMD_CDIAG_CMD, VSPEC1_MBOX_CMD_CMD);
	LOG_INFO("CDIAG: phy=%x, MDIO_MMD_VEND1=%x, VSPEC1_MBOX_ADDRLO=%x, addrlow=%x\n", (unsigned int)phy->phy_addr, (unsigned int)MDIO_MMD_VEND1, (unsigned int)VSPEC1_MBOX_ADDRLO, (unsigned int)addr_low);
...
int gpy2xx_sck_cdiag_result_read(struct gpy211_device *phy, int regno)
{
	int ret;
	u16 cmd = 0;
	LOG_INFO("Entering func name :- %s\n", __func__);
	...
	/* read Cable Diagnosis result  */
	FIELD_REPLACE(cmd, UTAPI_ID_CDIAG_RESULT_READ, VSPEC1_MBOX_CMD_ADDRHI);
	FIELD_REPLACE(cmd, MBOX_CMD_CDIAG_CMD, VSPEC1_MBOX_CMD_CMD);
	LOG_INFO("CDIAG: phy=%x, MDIO_MMD_VEND1=%x, VSPEC1_MBOX_ADDRLO=%x, regno=%x\n", (unsigned int)phy->phy_addr, (unsigned int)MDIO_MMD_VEND1, (unsigned int)VSPEC1_MBOX_ADDRLO, (unsigned int)regno);
	ret = PHY_WRITE_MMD(phy, MDIO_MMD_VEND1, VSPEC1_MBOX_ADDRLO, regno);
	LOG_INFO("CDIAG: phy=%x, MDIO_MMD_VEND1=%x, VSPEC1_MBOX_CMD=%x, cmd=%x\n", (unsigned int)phy->phy_addr, (unsigned int)MDIO_MMD_VEND1, (unsigned int)VSPEC1_MBOX_CMD, (unsigned int)cmd);
	ret = PHY_WRITE_MMD(phy, MDIO_MMD_VEND1, VSPEC1_MBOX_CMD, cmd);

	ret = PHY_READ_MMD(phy, MDIO_MMD_VEND1, VSPEC1_MBOX_ADDRLO);
	LOG_INFO("CDIAG: phy=%x, MDIO_MMD_VEND1=%x, VSPEC1_MBOX_ADDRLO=%x ret=%x\n", (unsigned int)phy->phy_addr, (unsigned int)MDIO_MMD_VEND1, (unsigned int)VSPEC1_MBOX_ADDRLO, (unsigned int)ret);
...
int gpy2xx_sck_cdiag_api_poll(struct gpy211_device *phy)
{
	int ret;
	u16 cmd = 0;
	LOG_INFO("Entering func name :- %s\n", __func__);
	...
	/* poll Cable Diagnosis status  */
	FIELD_REPLACE(cmd, UTAPI_ID_CDIAG_API_POLL, VSPEC1_MBOX_CMD_ADDRHI);
	FIELD_REPLACE(cmd, MBOX_CMD_CDIAG_CMD, VSPEC1_MBOX_CMD_CMD);
	LOG_INFO("CDIAG: phy=%x, MDIO_MMD_VEND1=%x, VSPEC1_MBOX_CMD=%x, cmd=%x\n", (unsigned int)phy->phy_addr, (unsigned int)MDIO_MMD_VEND1, (unsigned int)VSPEC1_MBOX_CMD, (unsigned int)cmd);
	ret = PHY_WRITE_MMD(phy, MDIO_MMD_VEND1, VSPEC1_MBOX_CMD, cmd);
...
int gpy2xx_sck_cdiag_api_exit(struct gpy211_device *phy)
{
	int ret;
	u16 cmd = 0;
	LOG_INFO("Entering func name :- %s\n", __func__);
	...
	/* exit Cable Diagnosis  */
	FIELD_REPLACE(cmd, UTAPI_ID_CDIAG_API_EXIT, VSPEC1_MBOX_CMD_ADDRHI);
	FIELD_REPLACE(cmd, MBOX_CMD_CDIAG_CMD, VSPEC1_MBOX_CMD_CMD);
	LOG_INFO("CDIAG: phy=%x, MDIO_MMD_VEND1=%x, VSPEC1_MBOX_CMD=%x, cmd=%x\n", (unsigned int)phy->phy_addr, (unsigned int)MDIO_MMD_VEND1, (unsigned int)VSPEC1_MBOX_CMD, (unsigned int)cmd);
	ret = PHY_WRITE_MMD(phy, MDIO_MMD_VEND1, VSPEC1_MBOX_CMD, cmd);
...
int gpy2xx_sck_cdiag_api_result_no(struct gpy211_device *phy)
{
	int ret;
	u16 cmd = 0;
	LOG_INFO("Entering func name :- %s\n", __func__);
	...
	/* read Cable Diagnosis result number */
	FIELD_REPLACE(cmd, UTAPI_ID_CDIAG_API_RESULT_NO, VSPEC1_MBOX_CMD_ADDRHI);
	FIELD_REPLACE(cmd, MBOX_CMD_CDIAG_CMD, VSPEC1_MBOX_CMD_CMD);
	LOG_INFO("CDIAG: phy=%x, MDIO_MMD_VEND1=%x, VSPEC1_MBOX_CMD=%x, cmd=%x\n", (unsigned int)phy->phy_addr, (unsigned int)MDIO_MMD_VEND1, (unsigned int)VSPEC1_MBOX_CMD, (unsigned int)cmd);
	ret = PHY_WRITE_MMD(phy, MDIO_MMD_VEND1, VSPEC1_MBOX_CMD, cmd);
...
int gpy2xx_fw_sram_dl_init(struct gpy211_device *phy, u32 timeout)
{
	int ret = -EINVAL;
	u32 fw_memory;
	...
	ret = PHY_READ(phy, PHY_PHY_STAT1);
	if (ret < 0) {
		printf("ERROR: PHY_STAT1 read fails\n");
		goto END;;
	}

	fw_memory = (ret >> 2) & 0x3;

	if (fw_memory == FW_EXECUTED_FROM_ROM) {

        LOG_INFO("Write mail box command CONST_VSPEC1_MBOX_CMD_CMD_SRAM_PATCH_REQ_FAST to init sram download");
 		ret = PHY_WRITE_MMD(phy, MDIO_MMD_VEND1, VSPEC1_MBOX_CMD, 0x0900);
		if (ret < 0) {
			LOG_CRIT("ERROR: VSPEC1_MBOX_CMD Write fail\n");
			goto END;
		}
	} 
	else {
		LOG_CRIT("ERROR: FW download can be initiated only if the memory is ROM\n");
		goto END;;
	}

	/*
		SWVL2 will Initialize MBOX_DATA_HIGH with pattern (0xA5A5) as an identification to host
		when entering CONST_VSPEC1_MBOX_CMD_CMD_SRAM_PATCH_REQ_FAST (FW download) mode
	*/
	do {
		udelay(500);
		...
		if (ret < 0) {
			printf("ERROR: Reading erase status fails\n");
			goto END;
		}

		timeout--;
	} while (!(ret == sram_download_init_success) && timeout > 0);

	if(ret != sram_download_init_success)
		ret = -EINVAL;

	LOG_INFO("Target ready for firmware download 0x%x\n", ret);
...
int gpy2xx_fw_sram_dl_write(struct gpy211_device *phy, u8 *buf, u32 size)
{
	int ret = -EINVAL;
	u16 data;
	u32 i;
	...
	{
		data = ((buf[i+1] << 0x8) | buf[i]);
		ret = PHY_WRITE_MMD(phy, MDIO_MMD_VEND1, VSPEC1_MBOX_CMD, data);
		if (ret < 0) {
			LOG_CRIT("ERROR: VSPEC1_MBOX_CMD data write fail\n");
			goto END;;
		}
	}
...
int gpy2xx_fw_sram_dl_uninit(struct gpy211_device *phy)
{
	int ret = -EINVAL;
	LOG_INFO("Entering func name :- %s\n", __func__);
...
int gpy2xx_super_isolate_mode_set(struct gpy211_device *phy, u8 enable)
{
	int ret = -EINVAL;
	u16 value = 0;
	LOG_INFO("Entering func name :- %s\n", __func__);
	...
	ret = PHY_READ_MMD(phy, MDIO_MMD_VEND1, VSPEC1_PM_CTRL);
	if (ret < 0) {
		printf("ERROR: Reading VSPEC1_PM_CTRL fails\n");
		goto END;
	}

	value = (u16)ret;
	FIELD_REPLACE(value, enable, VSPEC1_PM_CTRL_SI);

	ret = PHY_WRITE_MMD(phy, MDIO_MMD_VEND1, VSPEC1_PM_CTRL, value);
	if (ret < 0) {
		LOG_CRIT("ERROR: VSPEC1_PM_CTRL Write fail\n");
		goto END;
	}
...
int gpy2xx_super_isolate_mode_get(struct gpy211_device *phy)
{
	int ret = -EINVAL;
	u16 rd_data = 0;
	LOG_INFO("Entering func name :- %s\n", __func__);
	...
	rd_data = PHY_READ_MMD(phy, MDIO_MMD_VEND1, VSPEC1_PM_CTRL);
	if (rd_data < 0) {
		printf("ERROR: Reading VSPEC1_PM_CTRL fails\n");
		goto END;
	}

	ret = FIELD_GET(rd_data, VSPEC1_PM_CTRL_SI);
...
int gpy2xx_dvs_set(struct gpy211_device *phy, u8 disable)
{
	int ret = -EINVAL;
	u16 value = 0;
	LOG_INFO("Entering func name :- %s\n", __func__);
	...
	value = (u16)ret;
	FIELD_REPLACE(value, disable, VSPEC1_PM_CTRL_DVS_DIS);
...
int gpy2xx_dvs_get(struct gpy211_device *phy)
{
	int ret = -EINVAL;
	u16 rd_data = 0;
	LOG_INFO("Entering func name :- %s\n", __func__);
	...
	ret = FIELD_GET(rd_data, VSPEC1_PM_CTRL_DVS_DIS);
			 ...
			 const struct gpy211_macsec *msec_cfg)
{
	...
	/* MACsec init is in gpy211_macsec.c which is not built here. Matches v2.7.1.1 behavior. */
