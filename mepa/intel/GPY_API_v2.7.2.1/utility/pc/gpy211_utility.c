...
int gpy211_main(int argc, char *argv[])
{
	...
	if (xstrncasecmp(argv[0], "gpy2xx_init", strlen("gpy2xx_init")) == 0) {
	...
	} else if (xstrncasecmp(argv[0], "gpy2xx_sck_cdiag_start", strlen("gpy2xx_sck_cdiag_start")) == 0) {
		int ret;
		...
		return ret;  
	} else if (xstrncasecmp(argv[0], "gpy2xx_sck_cdiag_result_read", strlen("gpy2xx_sck_cdiag_result_read")) == 0) {
		int ret, loopcount = 0;
		u8 i;
		...
		return ret;  
	} else if (xstrncasecmp(argv[0], "gpy2xx_sram_download", strlen("gpy2xx_sram_download")) == 0) {
		char *fw_path = NULL;
		...
		fw_path = argv[1];
		clause = strtoul(argv[2], NULL, 0);
		cfw_ver = strtoul(argv[3], NULL, 0);
...
RESET_PHY:
		...
		if ((current_fw_version != cfw_ver) && (fw_memory != FW_EXECUTED_FROM_SRAM)) {
	...
	} else if (xstrncasecmp(argv[0], "gpy2xx_super_isolate_mode_set", strlen("gpy2xx_super_isolate_mode_set")) == 0) {
			u8 enable;
			...
			return ret;
	} else if (xstrncasecmp(argv[0], "gpy2xx_super_isolate_mode_get", strlen("gpy2xx_super_isolate_mode_get")) == 0) {
