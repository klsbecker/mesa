...
static int _lgm_reg_read(void *busdata, u32 regnum)
{
	int ret;
...
static int _lgm_get_clause(void *busdata, u32 phyaddr)
{
	GSW_MAC_Cli_t param = {0};
	struct gpy2xx_mdio *priv = (struct gpy2xx_mdio *)busdata;
...
static int _lgm_set_clause(void *busdata, u32 phyaddr, u32 clause)
{
	GSW_MAC_Cli_t param = {0};
	struct gpy2xx_mdio *priv = (struct gpy2xx_mdio *)busdata;
...
int gpy211_main(int argc, char *argv[])
{
	...
	if (xstrncasecmp(argv[2], "gpy2xx_uninit", strlen("gpy2xx_uninit")) == 0) {
	...
	} else if (xstrncasecmp(argv[2], "gpy2xx_super_isolate_mode_set", strlen("gpy2xx_super_isolate_mode_set")) == 0) {
		u8 enable;

		enable = strtoul(argv[3], NULL, 0);

		ret = gpy2xx_super_isolate_mode_set(&phy,enable);
		if (ret < 0)
			printf("\t%40s:\t0x%x", "gpy2xx_super_isolate_mode_set failed with ret code", ret);
		else
			printf("\t%40s", "gpy2xx_super_isolate_mode_set done");
		
		return ret;
	} else if (xstrncasecmp(argv[2], "gpy2xx_super_isolate_mode_get", strlen("gpy2xx_super_isolate_mode_get")) == 0) {

		ret = gpy2xx_super_isolate_mode_get(&phy);
		if (ret < 0)
			printf("\t%40s:\t0x%x", "gpy2xx_super_isolate_mode_get failed with ret code", ret);
		else
			printf("\t%40s %d", "super_isolate_mode status (1 - Enable , 0 - Disable) = ", ret);

	} else if (xstrncasecmp(argv[0], "gpy2xx_dvs_set", strlen("gpy2xx_dvs_set")) == 0) {
		u8 disable;

		disable = strtoul(argv[1], NULL, 0);

		ret = gpy2xx_dvs_set(&phy,disable);
		if (ret < 0)
			printf("\t%40s:\t0x%x", "gpy2xx_dvs_set failed with ret code", ret);
		else
			printf("\t%40s", "gpy2xx_dvs_set done");
		
		return ret;
	} else if (xstrncasecmp(argv[0], "gpy2xx_dvs_get", strlen("gpy2xx_dvs_get")) == 0) {
	
			ret = gpy2xx_dvs_get(&phy);
			if (ret < 0)
				printf("\t%40s:\t0x%x", "gpy2xx_dvs_get failed with ret code", ret);
			else
				printf("\t%40s %d", "DVS status (1 - Enable , 0 - Disable) = ", ret); 

	} else if (xstrncasecmp(argv[2], "gpy2xx_sck_cdiag_start", strlen("gpy2xx_sck_cdiag_start")) == 0) {
		int ret;
		ret = gpy2xx_sck_cdiag_start(&phy);
		if (ret < 0)
			printf("\t%40s:\t0x%x", "Gphy Cable Diag test start failed with ret code", ret);
		else
			printf("\t%40s", "Gphy Cable Diag test start done");

		return ret;
	} else if (xstrncasecmp(argv[2], "gpy2xx_sck_cdiag_result_read", strlen("gpy2xx_sck_cdiag_result_read")) == 0) {

		int ret, loopcount = 0;
		u8 i;
		struct gpy211_sck_cdiag_report cdiag_report;
		memset(&cdiag_report, 0, sizeof(struct gpy211_sck_cdiag_report));
		cdiag_report.register_number = gpy2xx_sck_cdiag_api_result_no(&phy);
		printf("\ncdiag_report.register_number: = %d.\n", cdiag_report.register_number);

		for (i = 0; i < cdiag_report.register_number; i++) {
			// poll CDiag status, if busy, then wait 10 ms
			ret = gpy2xx_sck_cdiag_api_poll(&phy);
			while (ret != 0) {
				ret = gpy2xx_sck_cdiag_api_poll(&phy);
				udelay(10);
				loopcount++;
			...
			switch (i) {
			case 0:
				cdiag_report.CDIAG_CTRL.V = gpy2xx_sck_cdiag_result_read(&phy, i);
				printf("\nCDIAG_CTRL.V: = %x RI=%x RA=%x DIS_IPAIR_CHECK=%x CDIAG_STATUS=%x CLU=%x C6DET=%x C5DET=%x LPDET=%x\n",
					    cdiag_report.CDIAG_CTRL.V, cdiag_report.CDIAG_CTRL.S.RI, cdiag_report.CDIAG_CTRL.S.RA,
					    cdiag_report.CDIAG_CTRL.S.DIS_IPAIR_CHECK, cdiag_report.CDIAG_CTRL.S.CDIAG_STATUS, cdiag_report.CDIAG_CTRL.S.CLU,
					    cdiag_report.CDIAG_CTRL.S.C6DET, cdiag_report.CDIAG_CTRL.S.C5DET, cdiag_report.CDIAG_CTRL.S.LPDET);
				break;
			case 1:
				cdiag_report.CDIAG_RESULT.V = gpy2xx_sck_cdiag_result_read(&phy, i);
				printf("\nPair Code Meaning: 0-Invalid  1-Ok  2-Open  3-IntraPairShort  4-InterPairShort  9-Busy.\n");
				printf("\nCDIAG_RESULT.V: = %x PAIRA_CDIAG_CODE=%x PAIRB_CDIAG_CODE=%x PAIRC_CDIAG_CODE=%x PAIRD_CDIAG_CODE=%x\n",
					    cdiag_report.CDIAG_RESULT.V, cdiag_report.CDIAG_RESULT.S.PAIRA_CDIAG_CODE, cdiag_report.CDIAG_RESULT.S.PAIRB_CDIAG_CODE,
					    cdiag_report.CDIAG_RESULT.S.PAIRC_CDIAG_CODE, cdiag_report.CDIAG_RESULT.S.PAIRD_CDIAG_CODE);
				break;
			case 2:
				cdiag_report.CDIAG_PAIR12LEN.V = gpy2xx_sck_cdiag_result_read(&phy, i);
				if (cdiag_report.CDIAG_CTRL.S.CLU == 1)
					printf("\nCDIAG_PAIR12LEN.V: = %d LENGTH=%dmeters \n",
						    cdiag_report.CDIAG_PAIR12LEN.V, cdiag_report.CDIAG_PAIR12LEN.S.LENGTH);
				else
					printf("\nCDIAG_PAIR12LEN.V: = %d LENGTH=%dcentimeters \n",
						    cdiag_report.CDIAG_PAIR12LEN.V, cdiag_report.CDIAG_PAIR12LEN.S.LENGTH);
				break;
			case 3:
				cdiag_report.CDIAG_PAIR36LEN.V = gpy2xx_sck_cdiag_result_read(&phy, i);
				if (cdiag_report.CDIAG_CTRL.S.CLU == 1)
					printf("\nCDIAG_PAIR36LEN.V: = %d LENGTH=%dmeters \n",
						    cdiag_report.CDIAG_PAIR36LEN.V, cdiag_report.CDIAG_PAIR36LEN.S.LENGTH);
				else
					printf("\nCDIAG_PAIR36LEN.V: = %d LENGTH=%dcentimeters \n",
						    cdiag_report.CDIAG_PAIR36LEN.V, cdiag_report.CDIAG_PAIR36LEN.S.LENGTH);
				break;
			case 4:
				cdiag_report.CDIAG_PAIR45LEN.V = gpy2xx_sck_cdiag_result_read(&phy, i);
				if (cdiag_report.CDIAG_CTRL.S.CLU == 1)
					printf("\nCDIAG_PAIR45LEN.V: = %d LENGTH=%dmeters \n",
						    cdiag_report.CDIAG_PAIR45LEN.V, cdiag_report.CDIAG_PAIR45LEN.S.LENGTH);
				else
					printf("\nCDIAG_PAIR45LEN.V: = %d LENGTH=%dcentimeters \n",
						    cdiag_report.CDIAG_PAIR45LEN.V, cdiag_report.CDIAG_PAIR45LEN.S.LENGTH);
				break;
			case 5:
				cdiag_report.CDIAG_PAIR78LEN.V = gpy2xx_sck_cdiag_result_read(&phy, i);
				if (cdiag_report.CDIAG_CTRL.S.CLU == 1)
					printf("\nCDIAG_PAIR78LEN.V: = %d LENGTH=%dmeters \n",
						    cdiag_report.CDIAG_PAIR78LEN.V, cdiag_report.CDIAG_PAIR78LEN.S.LENGTH);
				else
					printf("\nCDIAG_PAIR78LEN.V: = %d LENGTH=%dcentimeters \n",
						    cdiag_report.CDIAG_PAIR78LEN.V, cdiag_report.CDIAG_PAIR78LEN.S.LENGTH);
				break;
			case 6:
				cdiag_report.CDIAG_INSERTLOSSPAIR12.V = gpy2xx_sck_cdiag_result_read(&phy, i);
				printf("\nCDIAG_INSERTLOSSPAIR12.V: = %x LOSS50MHZ=%ddB LOSS100MHZ=%ddB LOSS200MHZ=%ddB LOSS400MHZ=%ddB\n",
					    cdiag_report.CDIAG_INSERTLOSSPAIR12.V, cdiag_report.CDIAG_INSERTLOSSPAIR12.S.LOSS50MHZ, cdiag_report.CDIAG_INSERTLOSSPAIR12.S.LOSS100MHZ,
					    cdiag_report.CDIAG_INSERTLOSSPAIR12.S.LOSS200MHZ, cdiag_report.CDIAG_INSERTLOSSPAIR12.S.LOSS400MHZ);
				break;
			case 7:
				cdiag_report.CDIAG_INSERTLOSSPAIR36.V = gpy2xx_sck_cdiag_result_read(&phy, i);
				printf("\nCDIAG_INSERTLOSSPAIR36.V: = %x LOSS50MHZ=%ddB LOSS100MHZ=%ddB LOSS200MHZ=%ddB LOSS400MHZ=%ddB\n",
					    cdiag_report.CDIAG_INSERTLOSSPAIR36.V, cdiag_report.CDIAG_INSERTLOSSPAIR36.S.LOSS50MHZ, cdiag_report.CDIAG_INSERTLOSSPAIR36.S.LOSS100MHZ,
					    cdiag_report.CDIAG_INSERTLOSSPAIR36.S.LOSS200MHZ, cdiag_report.CDIAG_INSERTLOSSPAIR36.S.LOSS400MHZ);
				break;
			case 8:
				cdiag_report.CDIAG_INSERTLOSSPAIR45.V = gpy2xx_sck_cdiag_result_read(&phy, i);
				printf("\nCDIAG_INSERTLOSSPAIR45.V: = %x LOSS50MHZ=%ddB LOSS100MHZ=%ddB LOSS200MHZ=%ddB LOSS400MHZ=%ddB\n",
					    cdiag_report.CDIAG_INSERTLOSSPAIR45.V, cdiag_report.CDIAG_INSERTLOSSPAIR45.S.LOSS50MHZ, cdiag_report.CDIAG_INSERTLOSSPAIR45.S.LOSS100MHZ,
					    cdiag_report.CDIAG_INSERTLOSSPAIR45.S.LOSS200MHZ, cdiag_report.CDIAG_INSERTLOSSPAIR45.S.LOSS400MHZ);
				break;
			case 9:
				cdiag_report.CDIAG_INSERTLOSSPAIR78.V = gpy2xx_sck_cdiag_result_read(&phy, i);
				printf("\nCDIAG_INSERTLOSSPAIR78.V: = %x LOSS50MHZ=%ddB LOSS100MHZ=%ddB LOSS200MHZ=%ddB LOSS400MHZ=%ddB\n",
					    cdiag_report.CDIAG_INSERTLOSSPAIR78.V, cdiag_report.CDIAG_INSERTLOSSPAIR78.S.LOSS50MHZ, cdiag_report.CDIAG_INSERTLOSSPAIR78.S.LOSS100MHZ,
					    cdiag_report.CDIAG_INSERTLOSSPAIR78.S.LOSS200MHZ, cdiag_report.CDIAG_INSERTLOSSPAIR78.S.LOSS400MHZ);
				break;
			case 10:
				cdiag_report.CDIAG_RETURNLOSSPAIR12.V = gpy2xx_sck_cdiag_result_read(&phy, i);
				printf("\nCDIAG_RETURNLOSSPAIR12.V: = %x LOSS50MHZ=%ddB LOSS100MHZ=%ddB LOSS200MHZ=%ddB LOSS400MHZ=%ddB\n",
					    cdiag_report.CDIAG_RETURNLOSSPAIR12.V, cdiag_report.CDIAG_RETURNLOSSPAIR12.S.LOSS50MHZ * 8, cdiag_report.CDIAG_RETURNLOSSPAIR12.S.LOSS100MHZ * 8,
					    cdiag_report.CDIAG_RETURNLOSSPAIR12.S.LOSS200MHZ * 8, cdiag_report.CDIAG_RETURNLOSSPAIR12.S.LOSS400MHZ * 8);
				break;
			case 11:
				cdiag_report.CDIAG_RETURNLOSSPAIR36.V = gpy2xx_sck_cdiag_result_read(&phy, i);
				printf("\nCDIAG_RETURNLOSSPAIR36.V: = %x LOSS50MHZ=%ddB LOSS100MHZ=%ddB LOSS200MHZ=%ddB LOSS400MHZ=%ddB\n",
					    cdiag_report.CDIAG_RETURNLOSSPAIR36.V, cdiag_report.CDIAG_RETURNLOSSPAIR36.S.LOSS50MHZ * 8, cdiag_report.CDIAG_RETURNLOSSPAIR36.S.LOSS100MHZ * 8,
					    cdiag_report.CDIAG_RETURNLOSSPAIR36.S.LOSS200MHZ * 8, cdiag_report.CDIAG_RETURNLOSSPAIR36.S.LOSS400MHZ * 8);
				break;
			case 12:
				cdiag_report.CDIAG_RETURNLOSSPAIR45.V = gpy2xx_sck_cdiag_result_read(&phy, i);
				printf("\nCDIAG_RETURNLOSSPAIR45.V: = %x LOSS50MHZ=%ddB LOSS100MHZ=%ddB LOSS200MHZ=%ddB LOSS400MHZ=%ddB\n",
					    cdiag_report.CDIAG_RETURNLOSSPAIR45.V, cdiag_report.CDIAG_RETURNLOSSPAIR45.S.LOSS50MHZ * 8, cdiag_report.CDIAG_RETURNLOSSPAIR45.S.LOSS100MHZ * 8,
					    cdiag_report.CDIAG_RETURNLOSSPAIR45.S.LOSS200MHZ * 8, cdiag_report.CDIAG_RETURNLOSSPAIR45.S.LOSS400MHZ * 8);
				break;
			case 13:
				cdiag_report.CDIAG_RETURNLOSSPAIR78.V = gpy2xx_sck_cdiag_result_read(&phy, i);
				printf("\nCDIAG_RETURNLOSSPAIR78.V: = %x LOSS50MHZ=%ddB LOSS100MHZ=%ddB LOSS200MHZ=%ddB LOSS400MHZ=%ddB\n",
					    cdiag_report.CDIAG_RETURNLOSSPAIR78.V, cdiag_report.CDIAG_RETURNLOSSPAIR78.S.LOSS50MHZ * 8, cdiag_report.CDIAG_RETURNLOSSPAIR78.S.LOSS100MHZ * 8,
					    cdiag_report.CDIAG_RETURNLOSSPAIR78.S.LOSS200MHZ * 8, cdiag_report.CDIAG_RETURNLOSSPAIR78.S.LOSS400MHZ * 8);
				break;
			case 14:
				cdiag_report.CDIAG_NEXTAB.V = gpy2xx_sck_cdiag_result_read(&phy, i);
				printf("\nCDIAG_NEXTAB.V: = %x LOSS50MHZ=%ddB LOSS100MHZ=%ddB LOSS200MHZ=%ddB LOSS400MHZ=%ddB\n",
					    cdiag_report.CDIAG_NEXTAB.V, cdiag_report.CDIAG_NEXTAB.S.LOSS50MHZ * 8, cdiag_report.CDIAG_NEXTAB.S.LOSS100MHZ * 8,
					    cdiag_report.CDIAG_NEXTAB.S.LOSS200MHZ * 8, cdiag_report.CDIAG_NEXTAB.S.LOSS400MHZ * 8);
				break;
			case 15:
				cdiag_report.CDIAG_NEXTAC.V = gpy2xx_sck_cdiag_result_read(&phy, i);
				printf("\nCDIAG_NEXTAC.V: = %x LOSS50MHZ=%ddB LOSS100MHZ=%ddB LOSS200MHZ=%ddB LOSS400MHZ=%ddB\n",
					    cdiag_report.CDIAG_NEXTAC.V, cdiag_report.CDIAG_NEXTAC.S.LOSS50MHZ * 8, cdiag_report.CDIAG_NEXTAC.S.LOSS100MHZ * 8,
					    cdiag_report.CDIAG_NEXTAC.S.LOSS200MHZ * 8, cdiag_report.CDIAG_NEXTAC.S.LOSS400MHZ * 8);
				break;
			case 16:
				cdiag_report.CDIAG_NEXTAD.V = gpy2xx_sck_cdiag_result_read(&phy, i);
				printf("\nCDIAG_NEXTAD.V: = %x LOSS50MHZ=%ddB LOSS100MHZ=%ddB LOSS200MHZ=%ddB LOSS400MHZ=%ddB\n",
					    cdiag_report.CDIAG_NEXTAD.V, cdiag_report.CDIAG_NEXTAD.S.LOSS50MHZ * 8, cdiag_report.CDIAG_NEXTAD.S.LOSS100MHZ * 8,
					    cdiag_report.CDIAG_NEXTAD.S.LOSS200MHZ * 8, cdiag_report.CDIAG_NEXTAD.S.LOSS400MHZ * 8);
				break;
			case 17:
				cdiag_report.CDIAG_NEXTBC.V = gpy2xx_sck_cdiag_result_read(&phy, i);
				printf("\nCDIAG_NEXTBC.V: = %x LOSS50MHZ=%ddB LOSS100MHZ=%ddB LOSS200MHZ=%ddB LOSS400MHZ=%ddB\n",
					    cdiag_report.CDIAG_NEXTBC.V, cdiag_report.CDIAG_NEXTBC.S.LOSS50MHZ * 8, cdiag_report.CDIAG_NEXTBC.S.LOSS100MHZ * 8,
					    cdiag_report.CDIAG_NEXTBC.S.LOSS200MHZ * 8, cdiag_report.CDIAG_NEXTBC.S.LOSS400MHZ * 8);
				break;
			case 18:
				cdiag_report.CDIAG_NEXTBD.V = gpy2xx_sck_cdiag_result_read(&phy, i);
				printf("\nCDIAG_NEXTBD.V: = %x LOSS50MHZ=%ddB LOSS100MHZ=%ddB LOSS200MHZ=%ddB LOSS400MHZ=%ddB\n",
					    cdiag_report.CDIAG_NEXTBD.V, cdiag_report.CDIAG_NEXTBD.S.LOSS50MHZ * 8, cdiag_report.CDIAG_NEXTBD.S.LOSS100MHZ * 8,
					    cdiag_report.CDIAG_NEXTBD.S.LOSS200MHZ * 8, cdiag_report.CDIAG_NEXTBD.S.LOSS400MHZ * 8);
				break;
			case 19:
				cdiag_report.CDIAG_NEXTCD.V = gpy2xx_sck_cdiag_result_read(&phy, i);
				printf("\nCDIAG_NEXTCD.V: = %x LOSS50MHZ=%ddB LOSS100MHZ=%ddB LOSS200MHZ=%ddB LOSS400MHZ=%ddB\n",
					    cdiag_report.CDIAG_NEXTCD.V, cdiag_report.CDIAG_NEXTCD.S.LOSS50MHZ * 8, cdiag_report.CDIAG_NEXTCD.S.LOSS100MHZ * 8,
					    cdiag_report.CDIAG_NEXTCD.S.LOSS200MHZ * 8, cdiag_report.CDIAG_NEXTCD.S.LOSS400MHZ * 8);
				break;
			default:
				printf("\nundefined register\n");
				break;
		...
		ret = gpy2xx_sck_cdiag_api_exit(&phy);
		if (ret < 0)
			printf("\t%40s:\t0x%x", "Gphy Cable Diag test start failed with ret code", ret);
		else
			printf("\t%40s", "Gphy Cable Diag test start done");
		...
		return ret;
	} else if (xstrncasecmp(argv[2], "gpy2xx_sram_download", strlen("gpy2xx_sram_download")) == 0) {
		char *fw_path = NULL;
		u32 cfw_ver, clause = 0;//cnt =0;
		//u32 mdc_clk_div = 0;
		FILE *fwin;
		u32 filesize;
		//u32 wrsize;
		u8 *buf;
		u16 sram_download_init_success=0xA5A5;
		u32 fw_memory,current_fw_version;

		if (argc < 5) {
			printf("\nERROR: gpy2xx_sram_download fail - insuffient parameters.\n");
			printf("\ngpy2xx_cli phyid gpy2xx_sram_download fw_path clausec sram_fw_ver\n");
		}

		fw_path = argv[3];
		clause = strtoul(argv[4], NULL, 0);
		cfw_ver = strtoul(argv[5], NULL, 0);

		printf("Download SRAM image %s\n",fw_path);
		...
		filesize = ftell(fwin);
		printf("SRAM image size: %u\n", filesize);
		rewind(fwin);

		if (clause == 0)
			printf("set clause45 id= %x\n", clause);
		else if (clause == 1)
			printf("set clause22 id= %x\n", clause);
		else {
			printf("set clause fail - unknown clause id = %x\n", clause);
		}
	
		_lgm_set_clause(phy.mdiobus_data, phy.phy_addr, clause & 1);
		printf("get LGM mdio clause is - %x\n", _lgm_get_clause(phy.mdiobus_data, phy.phy_addr));
		printf("get LGM mdio clause reg val - %x\n", _lgm_reg_read(phy.mdiobus_data, 0x220));

		printf("\n\ngpy2xx_fw_sram_dl_init\n");
		ret=gpy2xx_fw_sram_dl_init(&phy, FW_FWR_DEF_TIMEOUT);
		if (ret < 0) {
			printf("api gpy2xx_fw_sram_dl_init failed: %d\n", ret);
			fclose(fwin);
		}

		if(ret == sram_download_init_success){
			printf("Target ready for firmware download 0x%x - fw memory ROM\n ", ret);
		}
		else {
			printf("Target not ready for firmware download 0x%x - fw memory SRAM\n ", ret);
			return -errno;
		}

		buf = malloc(filesize);
		ret = fread(buf, 1, filesize, fwin);
		if (ret <= 0) {
		...
		if (ret != filesize) {
			printf("reading 'filesize' bytes from file failed: %d != %d\n", ret, filesize);
			fclose(fwin);
			free(buf);
			return -errno;
		}

		ret = gpy2xx_fw_sram_dl_write(&phy, buf, filesize);
		if (ret < 0) {
...
RESET_PHY:
		printf("\ngpy2xx_fw_sram_dl_uninit\n");
		printf("Reset Gphy\n\n");
		fflush(stdout);
		ret = gpy2xx_fw_sram_dl_uninit(&phy);
		if (ret < 0) {
		...
		fw_memory = (ret >> 2) & 0x3;
		current_fw_version=PHY_READ(&phy, PHY_PHY_FWV);

		if ((current_fw_version != cfw_ver) && (fw_memory != FW_EXECUTED_FROM_SRAM)) {
			printf("\nERROR: gpy2xx_sram_download FAIL -- sram fimware version = 0x%x\n", current_fw_version);
			printf("ERROR: gpy2xx_sram_download FAIL -- FW_EXECUTED_FROM_ROM = 0x%x\n", fw_memory);
		} else {
			printf("\ngpy2xx_sram_download PASS -- sram fimware version = 0x%x\n", current_fw_version);
			printf("gpy2xx_sram_download PASS -- FW_EXECUTED_FROM_SRAM = 0x%x\n", fw_memory);
		}	
		...
		if (phy.link.link == 0)
			...
			printf("\nLink Up.\n");
	}  else if (xstrncasecmp(argv[0], "gpy2xx_gpio_pad_vol_cfg_get", strlen("gpy2xx_gpio_pad_vol_cfg_get")) == 0) {
		u8 supplylevel=0;

		ret = gpy2xx_gpio_pad_vol_cfg_get(&phy);
		if (ret < 0)
			printf("\nERROR: Set gpy2xx_gpio_pad_vol_cfg failed.\n");

		supplylevel = ret;

		if(supplylevel)
			printf("\t%60s:\t0x%x\n", "HIGH (min 2.5v) val =", ret);
		else
			printf("\t%60s:\t0x%x\n", "LOW (max 1.8v) val =", ret);
	} else if (xstrncasecmp(argv[0], "gpy2xx_gpio_pad_vol_cfg", strlen("gpy2xx_gpio_pad_vol_cfg")) == 0) {
		u8 supplylevel=0;

		scanParamArg(argc, argv, "supplylevel", sizeof(supplylevel), &supplylevel);
		ret = gpy2xx_gpio_pad_vol_cfg(&phy, supplylevel);
		if (ret < 0)
			printf("\nERROR: Set gpy2xx_gpio_pad_vol_cfg failed.\n");
...
int init(int phyId)
{
	...
	printf("phy->id.OUI = 0x%08x\n",phy.id.OUI);
	printf("phy->id.model_no = 0x%x\n",phy.id.model_no);
	printf("phy->id.revision = 0x%x\n",phy.id.revision);
	printf("phy->id.fw_major = 0x%x\n",phy.id.fw_major);
	printf("phy->id.fw_minor = 0x%x\n",phy.id.fw_minor);
	printf("phy->id.fw_release = 0x%x\n",phy.id.fw_release);
