...
{
    u32 value, aneg_ena, tgt = VTSS_TO_DEV(vtss_state->port.map[port_no].chip_port);
    ...
    if (status->link == 0) {
        ...
        in_sync = SRVL_BF(DEV_PCS1G_CFG_STATUS_PCS1G_LINK_STATUS_SYNC_STATUS, value);
        status->link =
            SRVL_BF(DEV_PCS1G_CFG_STATUS_PCS1G_LINK_STATUS_LINK_STATUS, value) && in_sync;
    }
